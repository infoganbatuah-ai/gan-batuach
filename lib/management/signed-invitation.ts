...
export async function resolveSignedInvitation(admin: AdminClient, token: string) {
  ...
  const result = await admin.from("management_invitations")
    .select("id,invitation_type,intended_role,garden_id,target_profile_id,recipient_email,recipient_phone,status,expires_at,legacy_affiliation_request_id,staff_application_id,created_by,payload,gardens(name)")
