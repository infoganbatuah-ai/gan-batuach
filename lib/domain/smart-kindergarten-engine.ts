...
function promptsForRole(role: UserRole) {
  if (role === "parent") return ["מה התעדכן היום אצל הילד שלי?", "האם חסר לי מסמך?", "האם יש תשלום שדורש בדיקה?", "איך שולחים בקשה לגן?"];
  if (role === "staff") return ["מה המשימות שלי היום?", "אילו ילדים לעדכן?", "אילו מסמכים חסרים לי?", "מה חשוב עכשיו?"];
  if (role === "inspector") return ["מה לבדוק היום?", "אילו גנים מאחרים בפיקוח?", "אילו ליקויים פתוחים?", "אילו מצלמות לא מחוברות?"];
  return ["מה דורש טיפול היום?", "מי לא שילם?", "מי חסר בגדים?", "אילו פניות פתוחות?", "אילו מצלמות לא מחוברות?", "מה שכחתי?", "מה חשוב עכשיו?"];
}
