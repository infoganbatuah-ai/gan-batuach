...
export type EventCandidate = { event_type: string; evidence_kind?: string; confidence?: number; severity?: string; title?: string; description?: string; metadata?: Record<string, unknown> };
...
export class EventValidationPipeline {
  ...
  validate(candidate: EventCandidate, camera: Record<string, unknown>, zone?: Record<string, unknown> | null): ValidationResult {
    ...
    // A passive detection cannot inflate its own severity. The sole directional
    // exception is an authenticated line-crossing claim; the ingest route still
    // validates that the camera has a configured line before persistence.
    const verifiedCriticalCrossing = directionalEvents.has(eventType)
      && candidate.evidence_kind === "line_crossing" && requestedSeverity === "CRITICAL";
    const normalized = { ...candidate, event_type: eventType, severity: criticalEvents.has(eventType) || verifiedCriticalCrossing ? "CRITICAL" as const : passiveEvents.has(eventType) ? "INFO" as const : requestedSeverity === "INFO" ? "WARNING" as const : requestedSeverity };
    return { accepted: true, camera: mapped, event: normalized, shouldRecord: !healthEvents.has(eventType) && (actionable.has(normalized.severity) || ["fire_detected", "smoke_detected", "drowning_hazard", "pool_entry_off_hours", "unauthorized_entry", "lpr_unauthorized"].includes(eventType)) };
  }
}
