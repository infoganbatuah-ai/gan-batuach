...
export async function requireDigitalObserverAdmin(nextPath = "/digital-observer/admin") {
  ...
  const legacyGlobalAdmin = session.profile.role === "admin";

  if (!observerScoped && !legacyGlobalAdmin) redirect("/digital-observer/dashboard?error=observer_admin_required");
  if (!isAdminClientConfigured()) redirect("/digital-observer/dashboard?error=observer_admin_unavailable");

  return {
    ...session,
    observerAdmin: {
      scope: observerScoped ? "digital_observer_only" as const : "legacy_global_admin" as const,
