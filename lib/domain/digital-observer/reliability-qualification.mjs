...
export function summarizeRealHomeSoak(checkpoints, { startedAt, endedAt = Date.now(), requiredDurationMs = REAL_SOAK_MINIMUM_MS } = {}) {
  ...
  const perCamera = Object.fromEntries(expectedSources.map(name => {
    const samples = checkpoints.map(point => name === "tapo"
      ? point.tapo?.health_ok === true && finite(point.tapo?.progressing) === 1
      : point.dvr?.health_ok === true && (point.dvr?.inputs ?? []).some(input => `dvr-${input.channel}` === name));
    ...
    return [name, { availability: samples.filter(Boolean).length / samples.length, available_samples: samples.filter(Boolean).length, expected_samples: samples.length,
