...
export const QUALIFICATION_STAGE_MINIMUM_MS = Object.freeze({
  CANARY: 15 * 60 * 1_000,
  PRE_SOAK: 60 * 60 * 1_000,
  V8: REAL_SOAK_MINIMUM_MS
});
...
export function summarizeRealHomeSoak(checkpoints, { startedAt, endedAt = Date.now(),
  requiredDurationMs = REAL_SOAK_MINIMUM_MS, dvrSourceAvailable = 10,
  dvrKnownUpstreamUnavailable = [] } = {}) {
  ...
  const knownUnavailable = [...new Set(dvrKnownUpstreamUnavailable.map(Number))].sort((a, b) => a - b);
  if (!Number.isInteger(dvrSourceAvailable) || dvrSourceAvailable < 1 || dvrSourceAvailable > 10 ||
    knownUnavailable.some(channel => !canonicalDvrChannels.includes(channel)) ||
    dvrSourceAvailable + knownUnavailable.length !== 10)
    throw new Error("soak_dvr_source_availability_contract_invalid");
  const qualifiedDvrChannels = canonicalDvrChannels.filter(channel => !knownUnavailable.includes(channel));
  const qualifiedPhysicalCameras = dvrSourceAvailable + 1;
  const elapsedMs = Math.max(0, finite(endedAt) - finite(startedAt));
  const expectedCameraSeconds = checkpoints.length * qualifiedPhysicalCameras;
  const progressingCameraSamples = checkpoints.reduce((sum, point) => sum +
    Math.min(dvrSourceAvailable, finite(point.dvr?.progressing) ?? 0) + Math.min(1, finite(point.tapo?.progressing) ?? 0), 0);
  ...
  const componentAvailable = (point, kind) => point[kind]?.classification
    ? point[kind].classification === "PASS" : point[kind]?.health_ok === true;
  const gatewayUnavailable = checkpoints.filter(point => !componentAvailable(point, "dvr")).length;
  const tapoUnavailable = checkpoints.filter(point => !componentAvailable(point, "tapo")).length;
  ...
  const healthTransitions = kind => checkpoints.slice(1).reduce((sum, point, index) => sum +
    (componentAvailable(point, kind) !== componentAvailable(checkpoints[index], kind) ? 1 : 0), 0);
  const outageDurations = kind => {
    const durations = []; let outageStarted = null;
    for (const point of checkpoints) {
      const timestamp = Date.parse(point.sampled_at);
      if (!componentAvailable(point, kind) && outageStarted === null) outageStarted = timestamp;
      if (componentAvailable(point, kind) && outageStarted !== null) { durations.push(Math.max(expectedIntervalMs, timestamp - outageStarted)); outageStarted = null; }
    }
  ...
  const expectedSources = [...canonicalDvrChannels.map(channel => `dvr-${channel}`), "tapo"];
  const perCamera = Object.fromEntries(expectedSources.map(name => {
    ...
    const samples = checkpoints.map(point => {
      ...
      return input?.progressing === true ? true : input?.progressing === false || !componentAvailable(point, kind) ? false : null;
    });
    ...
    return [name, { qualification_denominator: name === "tapo" ||
      qualifiedDvrChannels.some(channel => name === `dvr-${channel}`),
      upstream_unavailable: knownUnavailable.some(channel => name === `dvr-${channel}`),
      availability: measured.length === samples.length ? measured.filter(Boolean).length / measured.length : null,
  ...
  if (playbackProbes.some(point => point.verified !== qualifiedPhysicalCameras)) gateFailures.push("PLAYBACK_PROBE_FAILED");
  ...
  if (knownUnavailable.length && checkpoints.some(point => point.dvr?.expected !== 10 ||
    point.dvr?.source_available !== dvrSourceAvailable ||
    JSON.stringify(point.dvr?.known_upstream_unavailable ?? []) !== JSON.stringify(knownUnavailable) ||
    point.dvr?.component_status !== "degraded"))
    gateFailures.push("DVR_SOURCE_AVAILABILITY_EXCEPTION_MISREPORTED");
  return Object.freeze({
    ...
    checkpoints: checkpoints.length, expected_physical_cameras: 11,
    source_available_physical_cameras: qualifiedPhysicalCameras,
    dvr_expected_physical_cameras: 10, dvr_source_available: dvrSourceAvailable,
    dvr_known_upstream_unavailable: knownUnavailable,
    configured_dvr_capacity: 16, empty_dvr_slots: 6,
    ...
    learning: { expected: qualifiedPhysicalCameras, sampled: learning.size, sampled_source_ids: [...learning].sort() },
...
export function assertQualificationStageResult(value, stage) {
  const minimum = QUALIFICATION_STAGE_MINIMUM_MS[stage];
  if (!minimum) throw new Error("qualification_stage_invalid");
  if (value?.required_elapsed_ms !== minimum) throw new Error("qualification_stage_duration_contract_invalid");
  if (value?.status !== "PASS" || value.required_elapsed_complete !== true || value.elapsed_ms < minimum ||
    !Array.isArray(value.gate_failures) || value.gate_failures.length) {
    throw new Error(`qualification_${stage.toLowerCase()}_gate_failed`);
  }
  return true;
}
