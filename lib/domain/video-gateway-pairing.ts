...
export function issueGatewayDiscoveryToken(input: Omit<GatewayPairingClaims, "version" | "scope" | "exp">, secret: string) {
  const payload: GatewayPairingClaims = { ...input, version: 1, scope: "cloud_discovery", exp: Date.now() + gatewayDiscoveryTokenTtlMs };
...
export function verifyGatewayDiscoveryToken(token: string, secret: string): GatewayPairingClaims | null {
  ...
  try {
    ...
    if (payload.version !== 1 || payload.scope !== "cloud_discovery" || !payload.pairing_id || !payload.gateway_id || !payload.observer_site_id || !Number.isFinite(payload.exp) || payload.exp <= Date.now()) return null;
    return payload;
  } catch {
    return null;
  }
