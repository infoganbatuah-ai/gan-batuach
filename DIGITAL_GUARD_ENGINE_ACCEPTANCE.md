...
| למידה לפי מצלמה | חוברו מדדי תנועה/תאורה ל-home-learning-sampler: הפרדה בין מצלמות, אזור זמן מהאתר, סינון דגימות לא תקינות/כפולות, השוואה לבסיס קודם, compare-and-swap ותור אירועים שמור לשחזור כתיבה. נוספה בדיקת כתיבה סגורה ל-Preview עם ניקוי מאומת, ללא הוצאת מפתח השרת. | קוד הבדיקה פרוס, אבל ניסיון הכתיבה נחסם 401 לפני יצירת נתונים. מסך QA עם כניסה רגילה מוכן לפריסת המשך. הבדיקות המקומיות מפעילות מסד מדומה; אינן הוכחת כתיבה ל-Supabase. |
...
| שחרור | commit `3e8e83f9ab229753e917e0bd18b45fd791635382` נדחף לענף המאושר ופריסתו אומתה Ready. 68 בדיקות עברו גם בעותק זה. helper שאילתות הצ׳אט ומסך QA נכתבו לאחר מכן ואינם בקומיט זה. | פריסת מסך QA ובדיקת הכתיבה החיה; שילוב הצ׳אט ובנייה מדויקת. אין אישור Production או מוכנות כללית. |

## בדיקות ותוצאות מדויקות

- ריצה משולבת חדשה: `node --test scripts/qa/digital-guard-runtime.test.mjs scripts/qa/digital-guard-preview.test.mjs scripts/qa/digital-guard-tenant-boundary.test.mjs scripts/qa/digital-guard-chat-query.test.mjs scripts/qa/digital-guard-qa-ui.test.mjs`: **88/88, קוד 0**. תשע בדיקות המסך החדשות מפעילות את רכיבי השרת והלקוח עם React/I/O מדומים: חסימה מחוץ ל-Preview ולחשבון QA, SHA, אישור מפורש, כפילות, session שפג, כשל ניקוי/תגובה ובלי retry. אינן תחליף לבדיקת דפדפן מחובר.
...
- פריסת הבדיקה הראשונה Ready. הוכן נתיב בדיקה בתוך השרת, כך שאין צורך למשוך או להעביר את המפתח למחשב. האישור הנוסף לשמירה ב-.env.local לא נוצל במשימה זו; הקובץ נבדק ונמצא מוחרג ואינו מנוהל ב-Git.
...
- Preview מאומת: `https://gan-batuach-pgmpry58d-gan-batuach-s-projects.vercel.app`, commit `3e8e83f9ab229753e917e0bd18b45fd791635382`, deployment `dpl_7adzhcppPBDWSTbKV45cY8fqKMVC`, מצב Ready.
- ניסיון runner עם התחברות QA רגילה החזיר **HTTP 401** ללא דוח fixture. GET הופנה לכניסת Vercel ולא נצפו לוגים של handler; אין ראיה שמפתח Supabase ב-Preview נדחה ואין ראיה לכתיבה חיה. לא נוצרו נתוני fixture בניסיון זה.
- `vercel curl` נדחה בביקורת ההרשאות משום שהוא עשוי ליצור/להשתמש במפתח עקיפה להגנת הפריסה. לא בוצע ולא נוסתה עקיפה או החלשת ההגנה.
- חלופה בטוחה: מסך `/digital-observer/qa/learning` עם כניסה רגילה ל-Vercel ולאפליקציה, guard זהה לשרת, אישור נתונים סינתטיים ו-SHA של הפריסה. הלקוח משתמש ב-session הרגיל בלבד; אין מפתח שרת בצד לקוח. בדיקות המסך ו-TypeScript עברו; פריסת המשך טרם אומתה.
- בדפדפן הגיע המסלול הרגיל למסך ההתחברות של האפליקציה. כניסת חשבון QA והרצת המסך עדיין לא הושלמו. אין לסמן כתיבה וניקוי חיים כהצלחה על סמך בדיקות מדומות.
