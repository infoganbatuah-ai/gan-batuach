...
import { softwareConnectorSecretStore, syncSoftwareConnectorConfiguration } from "../services/video-gateway/software-connector-cloud.mjs";
...
const synced = await syncSoftwareConnectorConfiguration(store).catch(() => ({ configured: false }));
process.env.GAN_BATUACH_GATEWAY_DISCOVERY = synced.configured ? "1" : "0";
