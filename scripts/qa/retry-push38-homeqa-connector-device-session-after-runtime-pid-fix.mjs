// Authorize one exact retry of the signed 0.2.17 Connector release after the
// launchd runtime-PID false-positive was corrected by a separately signed
// management-agent release. The installed OTA agent remains the sole installer.
...
import { PUSH38_CONNECTOR_DEVICE_SESSION_RECOVERY as item } from "../../services/video-gateway/push38-home-qa-connector-device-session.mjs";
...
const installedAdapterPath = join(root, "agent", "edge-macos-installed-adapter.mjs");
const fixedAdapterSha256 = "528d8178c0dcef34b366b191bad2e7bef52aed2cafc0926647236af696436e2f";
const managementReleaseId = "qa-p38-management-runtime-pid-95c3b60ed951";
const managementArtifactSha256 = "95c3b60ed951427d527d884af3b8d51ec2a24058a0f8b2672cdc28826d892dc3";
...
if (!mode || !outputPath.startsWith(restrictedRoot) || existsSync(outputPath))
  throw new Error("P38_CONNECTOR_DEVICE_SESSION_PIDFIX_MODE_OR_OUTPUT_INVALID");
...
function protectedFile(file) {
  if (!file || !existsSync(file) || !realpathSync(file).startsWith(restrictedRoot) ||
    ...
    throw new Error("P38_CONNECTOR_DEVICE_SESSION_PIDFIX_PROTECTED_EVIDENCE_REQUIRED");
...
async function sampleHealth() {
  ...
  const response = await fetch("http://127.0.0.1:18083/health", { signal: AbortSignal.timeout(10_000) });
  if (!response.ok) throw new Error("P38_CONNECTOR_DEVICE_SESSION_PIDFIX_RUNTIME_UNAVAILABLE");
...
if (!existsSync(configPath) || lstatSync(configPath).isSymbolicLink() ||
  ...
  throw new Error("P38_CONNECTOR_DEVICE_SESSION_PIDFIX_CONFIG_UNSAFE");
...
if (config.profile !== item.profile || config.deviceId !== item.deviceId || config.channel !== "HOME_QA" ||
  ...
  throw new Error("P38_CONNECTOR_DEVICE_SESSION_PIDFIX_CONFIG_MISMATCH");

const agentRelease = JSON.parse(readFileSync(agentReleasePath, "utf8"));
if (agentRelease.release_id !== managementReleaseId ||
  agentRelease.artifact_sha256 !== managementArtifactSha256 ||
  sha(readFileSync(installedAdapterPath)) !== fixedAdapterSha256)
  throw new Error("P38_CONNECTOR_DEVICE_SESSION_PIDFIX_MANAGEMENT_NOT_INSTALLED");
...
if (!verifyEdgeUpdateManifest(manifest, trusted).ok || manifest.release_id !== item.releaseId ||
  ...
  throw new Error("P38_CONNECTOR_DEVICE_SESSION_PIDFIX_MANIFEST_INVALID");
...
if (state.state !== "ROLLED_BACK" || state.release_id !== item.releaseId ||
  ...
  !manager.knownGood().some(entry => entry.release_id === item.rollbackReleaseId &&
  ...
  throw new Error("P38_CONNECTOR_DEVICE_SESSION_PIDFIX_ROLLBACK_STATE_INVALID");
...
const healthyEvents = readFileSync(agentLog, "utf8").split("\n").filter(Boolean).flatMap(line => {
...
if (healthyEvents.length < 15 || healthyDurationMs < 15 * 60_000)
  throw new Error("P38_CONNECTOR_DEVICE_SESSION_PIDFIX_PRIOR_HEALTH_INSUFFICIENT");
...
// The signed 0.2.14 rollback target predates the corrected discovery-health
// aggregation and can report connected=0 while the relay proves fresh frame
// progression. Require the stronger live-media evidence here; 0.2.17 must
// restore truthful discovery/health semantics before promotion.
if (samples.some(sample => !sample.running || !sample.pid || !["healthy", "degraded"].includes(sample.status) ||
  sample.expected !== 1 || sample.progressing !== 1 || sample.stalled !== 0) ||
  new Set(samples.map(sample => sample.pid)).size !== 1)
  throw new Error("P38_CONNECTOR_DEVICE_SESSION_PIDFIX_CURRENT_RUNTIME_UNSTABLE");
...
if (rollout.devices !== 2 || rollout.status !== "ACTIVE" || rollout.cohort_percent !== 0 ||
  ...
  throw new Error("P38_CONNECTOR_DEVICE_SESSION_PIDFIX_HOME_QA_INVALID");

const plan = { protocol: "observer-push38-connector-device-session-pidfix-retry-v1",
  ...
  management_release_id: managementReleaseId, fixed_adapter_sha256: fixedAdapterSha256,
  prior_healthy_samples: healthyEvents.length,
...
if (mode === "PREFLIGHT") {
  ...
  console.log(JSON.stringify({ status: "DEVICE_SESSION_PIDFIX_RETRY_PREFLIGHT_PASS",
    evidence_sha256: evidenceSha, release_id: item.releaseId, exact_device: true,
    broad_cohort: false, prior_healthy_seconds: Math.floor(healthyDurationMs / 1000), runtime_writes: 0 }));
  process.exit(0);
}

const planBytes = protectedFile(planPath);
if (!/^[a-f0-9]{64}$/.test(planSha256 || "") || sha(planBytes) !== planSha256)
  throw new Error("P38_CONNECTOR_DEVICE_SESSION_PIDFIX_PLAN_PIN_MISMATCH");
const saved = JSON.parse(planBytes);
if (saved.protocol !== plan.protocol || saved.release_id !== item.releaseId ||
  saved.artifact_sha256 !== item.digest || saved.management_release_id !== managementReleaseId ||
  Date.now() - Date.parse(saved.generated_at) > 10 * 60_000)
  throw new Error("P38_CONNECTOR_DEVICE_SESSION_PIDFIX_PLAN_STALE");
...
console.log(JSON.stringify({ status: "EXACT_DEVICE_SESSION_PIDFIX_RETRY_AUTHORIZED",
