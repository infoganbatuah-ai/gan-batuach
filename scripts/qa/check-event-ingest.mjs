...
const provenanceEvent={...event,event_id:randomUUID(),model_provenance:{model:"ssd_mobilenet_v1_10",runtime:"onnxruntime-node",execution_provider:"cpu",expected_sha256:"a".repeat(64)}};
assert.equal((await send(provenanceEvent)).status,201,"Authenticated real-model provenance is accepted by the strict event schema");
assert.deepEqual(tables.observer_intelligence_signals.at(-1).metadata.model_provenance,provenanceEvent.model_provenance,"Accepted provenance is persisted with the normalized event");
assert.equal((await send(event)).status,201);assert.equal(tables.observer_intelligence_signals.length,2,"Retries must not insert a second event");
...
assert.equal(tables.observer_intelligence_signals.at(-1).metadata.received_late,true);
...
assert.equal(tables.observer_intelligence_signals.length,10);
