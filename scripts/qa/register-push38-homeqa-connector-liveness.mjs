// Register the immutable AWS-signed Connector liveness recovery in the
// isolated PUSH 38 qualification database. Registration is exact-device and
// DRAFT-only; runtime activation remains a separate pre-write operation.
...
for (const path of [bundle, artifact, publication]) {
  ...
  if (!scoped || scoped === ".." || scoped.startsWith(`..${sep}`) || isAbsolute(scoped) ||
    ...
    throw new Error("P38_HOME_QA_LIVENESS_INPUT_SCOPE_INVALID");
}
...
if (!verifyEdgeUpdateManifest(document, keys).ok || document.release_id !== item.releaseId ||
  ...
  throw new Error("P38_HOME_QA_LIVENESS_RELEASE_VERIFICATION_FAILED");
if (!apply) {
...
if (labels["com.supabase.cli.project"] !== "gan-batuach-push38t" ||
  ...
  throw new Error("P38_HOME_QA_LIVENESS_DATABASE_NOT_ISOLATED");
...
do $$ begin
  ...
  then raise exception 'P38_HOME_QA_LIVENESS_RELEASE_CONFLICT'; end if;
end $$;
...
do $$ begin
  ...
  then raise exception 'P38_HOME_QA_LIVENESS_RECONCILIATION_FAILED'; end if;
end $$;
commit;`;
