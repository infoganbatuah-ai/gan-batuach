...
try {
  ...
  const incomplete=summarizeRealHomeSoak([{dvr:{health_ok:true,progressing:10,lifecycle:{}},tapo:{health_ok:true,progressing:1,lifecycle:{}},empty_dvr_slots:6,resources:{gateway:{pid:1,rss_mb:100},connector:{pid:2,rss_mb:50}},logs:{gateway_bytes:10,connector_bytes:10},playback:{verified:11,failed:0},ai:{ok:true,latency_ms:5},learning:{sampled_source_ids:Array.from({length:11},(_,i)=>`camera-${i}`)}}],{startedAt:Date.now()-60_000,endedAt:Date.now()});
