// Activate the exact AWS-signed Gateway session-stability remediation only
// after a protected, pinned preflight. The installed OTA agent remains the
// sole downloader/installer and signed 0.2.10 remains the rollback target.
...
import { PUSH38_GATEWAY_SESSION_STABILITY as item
} from "../../services/video-gateway/push38-home-qa-gateway-session-stability.mjs";
...
if (!bundleValue) throw new Error("P38_GATEWAY_SESSION_BUNDLE_REQUIRED");
const bundle = resolve(bundleValue);
const artifact = "/Volumes/DIGITAL_OBSERVER/Projects/Gan-Batuach/exports/restricted/push38-gateway-session-stability-9213c77c/gateway-runtime.tar.gz";
const publication = "/Volumes/DIGITAL_OBSERVER/Projects/Gan-Batuach/exports/restricted/push38-gateway-session-stability-9213c77c/r2-publication.json";
...
if (!mode || outputPath === resolve(".") || !outputPath.startsWith(restrictedRoot) || existsSync(outputPath))
  throw new Error("P38_GATEWAY_SESSION_MODE_OR_OUTPUT_INVALID");
...
function protectedFile(path) {
  if (!path || !existsSync(path) || !realpathSync(path).startsWith(restrictedRoot) ||
    ...
    throw new Error("P38_GATEWAY_SESSION_PROTECTED_EVIDENCE_REQUIRED");
...
function protectedLocalFile(path) {
  if (!path || !existsSync(path) || lstatSync(path).isSymbolicLink() || !lstatSync(path).isFile() ||
    ...
    throw new Error("P38_GATEWAY_SESSION_LOCAL_FILE_UNSAFE");
  return readFileSync(path);
}
...
function tlsProbe(path) {
  return new Promise((accept, reject) => {
    ...
    req.on("timeout", () => req.destroy(new Error("P38_GATEWAY_SESSION_TLS_TIMEOUT")));
...
async function healthSample(port, label) {
  ...
  if (!response.ok) throw new Error("P38_GATEWAY_SESSION_RUNTIME_UNAVAILABLE");
...
if (config.profile !== item.profile || config.deviceId !== item.deviceId || config.channel !== "HOME_QA" ||
  ...
  throw new Error("P38_GATEWAY_SESSION_CONFIG_MISMATCH");
if (sha(protectedLocalFile(config.qaTlsCaPath)) !== config.qaTlsCaSha256)
  throw new Error("P38_GATEWAY_SESSION_TLS_PIN_MISMATCH");
...
if (agentRelease.release_id !== item.releaseId || agentRelease.artifact_sha256 !== item.digest)
  throw new Error("P38_GATEWAY_SESSION_AGENT_RELEASE_MISMATCH");
...
if (!verifyEdgeUpdateManifest(manifest, trusted).ok || manifest.release_id !== item.releaseId ||
  ...
  throw new Error("P38_GATEWAY_SESSION_RELEASE_INVALID");
...
if (labels["com.supabase.cli.project"] !== "gan-batuach-push38t" || network !== "127.0.0.1")
  throw new Error("P38_GATEWAY_SESSION_DATABASE_NOT_ISOLATED");
...
if (current.release_id !== item.rollbackReleaseId ||
  ...
  throw new Error("P38_GATEWAY_SESSION_ROLLBACK_STATE_INVALID");
...
if (connectorCurrent.release_id !== connectorItem.releaseId ||
  ...
  throw new Error("P38_GATEWAY_SESSION_CONNECTOR_PREREQUISITE_INVALID");
...
if (rollout.devices !== 2 || rollout.releases !== 11 || rollout.new_status !== "DRAFT" ||
  ...
  throw new Error("P38_GATEWAY_SESSION_HOME_QA_STATE_INVALID");
...
if (!gatewayService.running || !gatewayService.pid || !gatewayAgent.running || !gatewayAgent.pid ||
  ...
  throw new Error("P38_GATEWAY_SESSION_SERVICE_MANAGER_INVALID");
...
if (gatewaySamples.some(sample => !sample.running || !sample.pid || sample.status !== "degraded" ||
  ...
  throw new Error("P38_GATEWAY_SESSION_RUNTIME_TRUTH_INVALID");
if (connectorSamples.some(sample => !sample.running || !sample.pid || !sample.ok ||
  ...
  throw new Error("P38_GATEWAY_SESSION_CONNECTOR_HEALTH_INVALID");
const disk = statfsSync(root);
if (Number(disk.bavail) * Number(disk.bsize) < item.size * 3)
  throw new Error("P38_GATEWAY_SESSION_DISK_INSUFFICIENT");
...
if (anonymous !== 401 || wrongRoute !== 404)
  throw new Error("P38_GATEWAY_SESSION_INGRESS_INVALID");

const plan = { protocol: "observer-push38-gateway-session-stability-activation-v1",
...
if (!/^[a-f0-9]{64}$/.test(planSha) || sha(savedBytes) !== planSha)
  throw new Error("P38_GATEWAY_SESSION_PLAN_PIN_MISMATCH");
...
if (saved.protocol !== plan.protocol || saved.release_id !== item.releaseId ||
  ...
  throw new Error("P38_GATEWAY_SESSION_PLAN_STALE");
const sql = `begin;
  ...
  do $$ begin
    ...
    then raise exception 'P38_GATEWAY_SESSION_ACTIVATION_VERIFY_FAILED'; end if;
  end $$;
commit;`;
...
console.log(JSON.stringify({ status: "EXACT_GATEWAY_SESSION_ROLLOUT_ACTIVE",
  evidence_sha256: evidenceSha, release_id: item.releaseId, exact_device: true,
