...
const adapter = { async install({ staging, manifest: value }) { writeFileSync(join(staging, "installed-version"), value.version); }, async restart() {} };
function manager(profile, healthCheck) {
  ...
  value.initializeKnownGood(); return { value, root, identityPath, identity: readFileSync(identityPath, "utf8") };
}
...
const connector = manager("SOFTWARE_CONNECTOR", async () => healthy(1, 0));
...
const tamper = manager("SOFTWARE_CONNECTOR", async () => healthy(1));
...
const interrupted = manager("SOFTWARE_CONNECTOR", async () => healthy(1));
...
const gateway = manager("PHYSICAL_GATEWAY", async () => healthy(10, 6));
