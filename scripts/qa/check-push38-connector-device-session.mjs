...
import {
  ...
  PUSH38_CONNECTOR_DEVICE_SESSION_RECOVERY as item
} from "../../services/video-gateway/push38-home-qa-connector-device-session.mjs";

const origin = "https://693f824a750afcc264fe6ee58c8a86ab.r2.cloudflarestorage.com";
const { document } = buildPush38ConnectorDeviceSessionRecoveryManifest({
  signingKeyId: "observer-kms-release-v1", artifactOrigin: origin,
  releasedAt: new Date().toISOString()
});
assert.equal(document.release_id, item.releaseId);
assert.equal(document.version, "0.2.17-p38-health");
assert.equal(document.build_sha, "005319bf75762847ebd0c00613d82fbb469f4521");
...
assert.throws(() => buildPush38ConnectorDeviceSessionRecoveryManifest({ signingKeyId: "bad key",
  artifactOrigin: origin, releasedAt: new Date().toISOString() }), /SIGNER_INVALID/);
assert.throws(() => buildPush38ConnectorDeviceSessionRecoveryManifest({ signingKeyId: "valid-key",
