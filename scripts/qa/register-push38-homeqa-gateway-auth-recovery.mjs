// Registers and, only after an exact managed-identity proof, activates the
// corrective Gateway release in the isolated HOME_QA database.
...
if (activate && !apply) throw new Error("P38_GATEWAY_AUTH_ACTIVATION_REQUIRES_APPLY");
...
for (const path of [bundle, artifact]) {
  ...
  if (!scoped || scoped === ".." || scoped.startsWith(`..${sep}`) || isAbsolute(scoped) ||
    ...
    throw new Error("P38_GATEWAY_AUTH_INPUT_SCOPE_INVALID");
}
...
const document = JSON.parse(execFileSync("unzip", ["-p", bundle, "gateway_remediation_auth.json"],
...
if (!verifyEdgeUpdateManifest(document, keys).ok || document.release_id !== item.releaseId ||
  ...
  assertEdgeReleaseObjectUrl(document, origin) !== `home-qa/${item.releaseId}/${item.digest}.tar.gz` ||
  statSync(artifact).size !== item.size || await hash(artifact) !== item.digest)
  throw new Error("P38_GATEWAY_AUTH_RELEASE_VERIFICATION_FAILED");
if (!apply) {
...
const targetStatus = activate ? "ACTIVE" : "DRAFT";
const sql = `begin;
do $$ begin
  if (select count(*) from public.video_gateway_device_enrollments) <> 2 or
     ...
     not exists(select 1 from public.observer_edge_releases where release_id='qa-p38-health-gateway-6c9d08327ec6')
  then raise exception 'P38_GATEWAY_AUTH_PREREQUISITE_MISSING'; end if;
end $$;
...
do $$ begin
  ...
  then raise exception 'P38_GATEWAY_AUTH_RELEASE_CONFLICT'; end if;
end $$;
...
where release_id in (select id from public.observer_edge_releases where deployment_profile='PHYSICAL_GATEWAY'
  and release_id<>'${item.releaseId}') and status in ('DRAFT','ACTIVE');
${activate ? `update public.observer_edge_rollouts set status='ACTIVE'
  where release_id=(select id from public.observer_edge_releases where release_id='${item.releaseId}')
  and status='DRAFT';` : ""}
do $$ begin
  if (select count(*) from public.observer_edge_releases where channel='HOME_QA') <> 7 or
     not exists(select 1 from public.observer_edge_rollouts o join public.observer_edge_releases r on r.id=o.release_id
       where r.release_id='${item.releaseId}' and o.status='${targetStatus}' and o.cohort_percent=0
     ...
     exists(select 1 from public.observer_edge_rollouts o join public.observer_edge_releases r on r.id=o.release_id
       where r.deployment_profile='PHYSICAL_GATEWAY' and r.release_id<>'${item.releaseId}' and o.status<>'PAUSED') or
     exists(select 1 from public.observer_edge_rollouts where cohort_percent<>0)
  then raise exception 'P38_GATEWAY_AUTH_RECONCILIATION_FAILED'; end if;
end $$;
commit;`;
...
console.log(JSON.stringify({ status: activate ? "GATEWAY_AUTH_RECOVERY_ACTIVE" : "GATEWAY_AUTH_RECOVERY_REGISTERED_DRAFT",
  release_id: item.releaseId, superseded_gateway_release: "PAUSED", exact_device: true,
  broad_cohort: "DISABLED", production_writes: 0 }));
