...
const pidfix = JSON.parse(run(process.execPath, ["scripts/qa/register-push38-homeqa-connector-pidfix.mjs"], 60_000));
const raw = run("docker", ["--context", "colima-push38t", "exec", "supabase_db_gan-batuach-push38t",
  ...
  "(select count(*) from public.observer_edge_rollouts o join public.observer_edge_releases r on r.id=o.release_id where r.release_id='qa-p38-health-connector-pidfix-1b9e9499ffa7' and o.status='DRAFT')," +
  "(select count(*) from public.observer_edge_rollouts o join public.observer_edge_releases r on r.id=o.release_id where r.release_id='qa-p38-health-connector-1b076f596574' and o.status='PAUSED')," +
...
const [devices, registered, active, remediationDraft, supersededPaused, authorizedDevices] = raw.split("|").map(Number);
...
const gates = {
  ...
  signed_artifacts: releases.status === "VERIFIED_NOT_REGISTERED" && releases.artifact_hashes &&
    pidfix.status === "VERIFIED_NOT_REGISTERED" && pidfix.exact_device === true,
  ...
  qa_releases: registered === 4,
  qa_exact_active_rollouts: active === 2 && remediationDraft === 1 && supersededPaused === 1,
...
const plan = { protocol: "observer-push38-home-qa-live-plan-v1", generated_at: new Date().toISOString(),
  ...
  gates, qa_counts: { devices, registered_releases: registered, active_rollouts: active,
    remediation_draft: remediationDraft, superseded_remediation_paused: supersededPaused,
