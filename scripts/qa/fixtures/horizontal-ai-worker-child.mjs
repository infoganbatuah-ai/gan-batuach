...
const queue = createDurableAiJobQueue({ databasePath, policy: { leaseMs: 30_000 }, workerAuthorizer: worker => worker.identity?.device_id?.startsWith("qa-worker-") === true });
...
try {
  // Allow peers to finish their pending work before reporting a benchmark.
  // Interrupted-lease recovery is tested separately with a 100 ms lease.
  while (idle < 50) {
