  ...
  while (idle < 50) {
    ...
    if (result.status === "IDLE") { idle++; await new Promise(resolve => setTimeout(resolve, 10)); }
      ...
      if (result.status === "COMPLETED") {
        completed++;
        completedJobIds.add(result.job_id);
      }
    }
  }
  const cpu = process.cpuUsage(cpuStart);
  process.stdout.write(JSON.stringify({ worker_id: workerId, completed, completed_job_ids: [...completedJobIds], worker_cpu_ms: Number(((cpu.user + cpu.system) / 1000).toFixed(3)), snapshot: worker.snapshot() }));
} finally { queue.close(); }
