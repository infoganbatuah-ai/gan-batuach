// Register the immutable AWS-signed Gateway session-stability remediation in
...
if (!bundleValue) throw new Error("P38_GATEWAY_SESSION_HOME_QA_BUNDLE_REQUIRED");
const bundle = resolve(bundleValue);
const artifact = `${restrictedRoot}/push38-gateway-session-stability-9213c77c/gateway-runtime.tar.gz`;
const publication = `${restrictedRoot}/push38-gateway-session-stability-9213c77c/r2-publication.json`;
const bundleName = "gateway_remediation_session_stability.json";
const expectedBefore = 10, expectedAfter = 11;
...
for (const path of [bundle, artifact, publication]) {
  ...
  if (!scoped || scoped === ".." || scoped.startsWith(`..${sep}`) || isAbsolute(scoped) ||
    ...
    throw new Error("P38_GATEWAY_SESSION_HOME_QA_INPUT_SCOPE_INVALID");
}
...
if (!verifyEdgeUpdateManifest(document, keys).ok || document.release_id !== item.releaseId ||
  ...
  throw new Error("P38_GATEWAY_SESSION_HOME_QA_RELEASE_VERIFICATION_FAILED");
if (!apply) {
...
if (labels["com.supabase.cli.project"] !== "gan-batuach-push38t" ||
  ...
  throw new Error("P38_GATEWAY_SESSION_HOME_QA_DATABASE_NOT_ISOLATED");
...
do $$ begin
  ...
  then raise exception 'P38_GATEWAY_SESSION_HOME_QA_PREREQUISITE_MISSING'; end if;
end $$;
...
do $$ begin
  ...
  then raise exception 'P38_GATEWAY_SESSION_HOME_QA_RELEASE_CONFLICT'; end if;
end $$;
...
do $$ begin
  ...
  then raise exception 'P38_GATEWAY_SESSION_HOME_QA_RECONCILIATION_FAILED'; end if;
end $$;
commit;`;
...
console.log(JSON.stringify({ status: "GATEWAY_SESSION_STABILITY_REGISTERED_DRAFT",
