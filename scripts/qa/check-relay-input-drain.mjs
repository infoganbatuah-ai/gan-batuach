...
const fixture = spawnSync("ffmpeg", ["-hide_banner", "-loglevel", "error", "-filter_threads", "1", "-f", "lavfi", "-i", "testsrc=size=160x96:rate=25", "-frames:v", "130", "-c:v", "libx264", "-threads", "1", "-preset", "ultrafast", "-pix_fmt", "yuv420p", "-g", "25", "-f", "mp4", "-movflags", "frag_keyframe+empty_moov", "pipe:1"], { timeout: 15000, maxBuffer: 2_000_000 });
...
const context = vm.createContext({
  ...
  streamSources: new Map([["fixture", { kind: "private_nvr_http_mp4", codec: "h264" }]]),
  relayDirectory: () => root, mkdirSync, existsSync, statSync, join,
  hardwareTranscoder: { test() { assert.fail("H264 copy needs no hardware provisioning"); } },
...
try {
  ...
  assert.match(playlist, /#EXTINF:0\.200000/);
  console.log(JSON.stringify({ result: "PASS", final_window_seconds: duration, socket_errors: 1, synthetic_only: true }));
} finally {
