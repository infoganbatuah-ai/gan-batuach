...
import { buildPush38GatewayAuthRecoveryManifest } from "../../services/video-gateway/push38-home-qa-gateway-auth-recovery.mjs";
...
async function hashStream(stream, limit) {
  ...
  for await (const chunk of stream) {
    size += chunk.length;
    if (size > limit) fail("P38_GATEWAY_AUTH_R2_SIZE_LIMIT");
    hash.update(chunk);
  }
  return { sha256: hash.digest("hex"), size };
}

export async function publishPush38GatewayAuthRecovery({ artifactPath, evidencePath }) {
  const { document } = buildPush38GatewayAuthRecoveryManifest({ signingKeyId: "observer-kms-release-v1",
  ...
  if (!artifactRelative || artifactRelative === ".." || artifactRelative.startsWith(`..${sep}`) ||
    ...
    fail("P38_GATEWAY_AUTH_R2_LOCAL_ARTIFACT_INVALID");
  ...
  if (local.sha256 !== document.artifact_sha256 || local.size !== document.artifact_size)
    fail("P38_GATEWAY_AUTH_R2_LOCAL_ARTIFACT_HASH_MISMATCH");
  ...
  try {
    ...
    if (head.ContentLength !== local.size || head.Metadata?.sha256 !== local.sha256 ||
      head.Metadata?.release_id !== document.release_id) fail("P38_GATEWAY_AUTH_R2_EXISTING_OBJECT_CONFLICT");
    ...
    if (url.origin !== origin || url.pathname !== `/${EDGE_RELEASE_R2_BUCKET}/${key}` ||
      url.searchParams.get("X-Amz-Expires") !== "120") fail("P38_GATEWAY_AUTH_R2_CAPABILITY_SCOPE_INVALID");
    ...
    if (!response.ok || Number(response.headers.get("content-length")) !== local.size)
      fail("P38_GATEWAY_AUTH_R2_DOWNLOAD_INVALID");
    const downloaded = await hashStream(response.body, local.size);
    if (downloaded.sha256 !== local.sha256 || downloaded.size !== local.size)
      fail("P38_GATEWAY_AUTH_R2_ROUND_TRIP_MISMATCH");
    const anonymous = await fetch(`${origin}/${EDGE_RELEASE_R2_BUCKET}/${key}`, { redirect: "error",
      signal: AbortSignal.timeout(30_000) });
    await anonymous.body?.cancel();
    if (anonymous.ok) fail("P38_GATEWAY_AUTH_R2_PUBLIC_ACCESS_ENABLED");
    const result = { protocol: "observer-push38-gateway-auth-recovery-r2-publication-v1",
...
if (process.argv[1] && fileURLToPath(import.meta.url) === resolve(process.argv[1])) {
  try {
    ...
    if (!artifact || !evidence || !scoped || scoped === ".." || scoped.startsWith(`..${sep}`) || isAbsolute(scoped))
      fail("P38_GATEWAY_AUTH_R2_INPUT_SCOPE_INVALID");
    console.log(JSON.stringify({ result: "PASS", publication: await publishPush38GatewayAuthRecovery({
      artifactPath: artifact, evidencePath: resolve(evidence) }) }));
  } catch (error) {
    console.error(/^P38_GATEWAY_AUTH_R2_[A-Z0-9_]+$/.test(error.message) ? error.message :
      "P38_GATEWAY_AUTH_R2_PUBLICATION_FAILED");
    process.exitCode = 1;
  }
}
