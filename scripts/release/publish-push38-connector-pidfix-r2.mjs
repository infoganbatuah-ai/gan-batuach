...
async function publish({ artifactPath, evidencePath, recovery = false, startupRecovery = false,
  ...
  hostContinuityRecovery = false }) {
  const builder = hostContinuityRecovery ? buildPush38ConnectorHostContinuityRecoveryManifest :
  ...
  try {
    ...
    const result = { protocol: hostContinuityRecovery ? "observer-push38-host-continuity-recovery-r2-publication-v1" :
  ...
  try {
    ...
    const [artifact, evidence] = process.argv.slice(2)
      .filter(value => !["--health-recovery", "--startup-recovery", "--liveness-recovery",
        "--parent-exit-recovery", "--rtsp-session-recovery", "--host-continuity-recovery"].includes(value));
    ...
    console.log(JSON.stringify({ result: "PASS", publication: await publish({ artifactPath: artifact,
      ...
      rtspSessionRecovery, hostContinuityRecovery }) }));
  } catch (error) {
