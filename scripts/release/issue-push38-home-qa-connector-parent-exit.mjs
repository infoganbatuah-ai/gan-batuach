...
export async function issuePush38ConnectorParentExitRecovery({ env = process.env, call }) {
  if (env.GITHUB_REPOSITORY !== "infoganbatuah-ai/gan-batuach" ||
    env.GITHUB_REF !== "refs/heads/codex/push-38-aws-signing" ||
    env.PUSH38_CANDIDATE_SHA !== PUSH38_CONNECTOR_PARENT_EXIT_RECOVERY.buildSha ||
    !env.RUNNER_TEMP || !env.HOME_QA_OUTPUT_DIR)
    fail("P38_PARENT_EXIT_SIGNING_CONTEXT_INVALID");
  ...
  if (!output.startsWith(`${runner}/`) || existsSync(output) || lstatSync(runner).isSymbolicLink())
    fail("P38_PARENT_EXIT_OUTPUT_SCOPE_INVALID");
  ...
  if (publicResult.KeyId !== config.keyArn || typeof publicResult.PublicKey !== "string")
    fail("P38_PARENT_EXIT_PUBLIC_KEY_INVALID");
  const publicBytes = Buffer.from(publicResult.PublicKey, "base64");
  if (digest(publicBytes) !== config.publicKeySha256) fail("P38_PARENT_EXIT_PUBLIC_KEY_PIN_MISMATCH");
  const item = buildPush38ConnectorParentExitRecoveryManifest({ signingKeyId: config.keyId,
  ...
  if (!verifyEdgeUpdateManifest(result.document, { [config.keyId]: publicBytes.toString("base64url") }).ok)
    fail("P38_PARENT_EXIT_SIGNED_MANIFEST_INVALID");
  mkdirSync(output, { mode: 0o700 });
  const bytes = Buffer.from(`${JSON.stringify(result.document, null, 2)}\n`);
  writeFileSync(join(output, "connector_remediation_parent_exit.json"), bytes, { flag: "wx", mode: 0o600 });
  writeFileSync(join(output, "connector_remediation_parent_exit.evidence.json"),
...
if (process.argv[1] && fileURLToPath(import.meta.url) === resolve(process.argv[1])) {
  const temporary = mkdtempSync(join(tmpdir(), "observer-p38-parent-exit-sign-"));
  let sequence = 0;
  try {
    ...
    const call = async (operation, input) => {
      ...
      catch { fail("P38_PARENT_EXIT_KMS_REQUEST_FAILED"); }
    };
    console.log(JSON.stringify({ result: "PASS",
      manifest: await issuePush38ConnectorParentExitRecovery({ call }) }));
  } catch (error) {
    console.error(/^P38_PARENT_EXIT_[A-Z0-9_]+$/.test(error.message) ? error.message :
      "P38_PARENT_EXIT_ISSUANCE_FAILED");
    process.exitCode = 1;
