-- GB-M29 isolated RLS QA follow-up: preserve the original messaging model while
-- closing classroom scope, direct table privileges, and same-key concurrency gaps.
-- The three RPC bodies are repeated because applied migration files are immutable.

revoke truncate, references, trigger on public.communication_threads,
  public.communication_thread_participants, public.communication_delivery_events,
  public.messages from anon, authenticated;
...
begin
  ...
  if actor.role::text='parent' then
    ...
    target_is_manager := exists(
      select 1 from public.gardens g
      join public.garden_management_memberships membership
        on membership.garden_id=g.id and membership.profile_id=recipient.id
        and membership.status='active' and membership.ended_at is null
        and membership.relationship_role in ('manager','owner')
      where g.id=garden and (g.manager_id=recipient.id or g.owner_profile_id=recipient.id)
        and recipient.role::text in ('manager','owner')
    );
    target_is_staff := exists(
      ...
      join public.staff_classroom_assignments sa on sa.staff_id=s.id and sa.garden_id=garden and sa.status='active'
      join public.child_classroom_assignments ca on ca.classroom_id=sa.classroom_id
        and ca.garden_id=garden and ca.child_id=child_row.id and ca.is_current
      where e.profile_id=recipient.id and e.garden_id=garden and e.status='active'
...
-- PostgreSQL functions are executable by PUBLIC by default; authenticated-only
-- RPCs must not remain callable by anon even though their actor guard fails shut.
revoke execute on function public.create_management_communication_thread(uuid,uuid,uuid,text,text,uuid),
  public.send_management_communication_message(uuid,text,uuid),
  public.create_management_communication_broadcast(uuid,uuid,text,text,text,uuid),
  public.mark_management_communication_thread_read(uuid)
from public, anon;
grant execute on function public.create_management_communication_thread(uuid,uuid,uuid,text,text,uuid),
  public.send_management_communication_message(uuid,text,uuid),
  public.create_management_communication_broadcast(uuid,uuid,text,text,text,uuid),
  public.mark_management_communication_thread_read(uuid)
to authenticated;
