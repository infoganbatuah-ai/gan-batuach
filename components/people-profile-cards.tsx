  ...
  return (
    <section className="dashboard-section people-directory">
      ...
      {rows.length === 0 ? <div className="empty-state"><strong>אין הורים להצגה</strong><span>הוסיפו הורה דרך תהליך הקליטה כדי לפתוח משתמש ולהתחיל רישום ילד.</span><Link className="button primary" href="/dashboard/garden/onboarding">הוספת הורה</Link></div> : <div className="people-card-grid">{rows.map((parent) => {
        ...
        <div className="profile-actions"><Link className="button secondary tiny" href="/dashboard/garden/messages"><MessageCircle size={14} /> הודעה</Link><a className="button secondary tiny" href={parent.phone ? `tel:${parent.phone}` : undefined}><Phone size={14} /> שיחה</a>{credential && !passwordChanged ? <button className="button secondary tiny" type="button" onClick={() => navigator.clipboard?.writeText(`${credential.username}\n${credential.temporary_password}`)}>העתקת כניסה</button> : <Link className="button secondary tiny" href="/dashboard/garden/parents">שליחת איפוס סיסמה</Link>}<Link className="button tiny" href={`/dashboard/garden/parents?parent=${parent.id}`}>פרופיל</Link></div>
      </article>; })}</div>}
    </section>
  );
}
