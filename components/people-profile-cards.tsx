  ...
  return (
    <section className="dashboard-section people-directory">
      ...
      {rows.length === 0 ? <div className="empty-state"><strong>אין הורים להצגה</strong><span>הוסיפו הורה דרך תהליך הקליטה כדי לפתוח משתמש ולהתחיל רישום ילד.</span><Link className="button primary" href="/dashboard/garden/onboarding">הוספת הורה</Link></div> : <div className="people-card-grid">{rows.map((parent) => {
        ...
        <div className="credential-box compact"><b>{parent.email ?? "אין כתובת דוא״ל"}</b><span>שחזור סיסמה מתבצע דרך חשבון ההורה בלבד.</span></div>
        ...
        <div className="profile-actions"><Link className="button secondary tiny" href="/dashboard/garden/messages"><MessageCircle size={14} /> הודעה</Link>{parent.phone ? <a className="button secondary tiny" href={`tel:${parent.phone}`}><Phone size={14} /> שיחה</a> : <button className="button secondary tiny" type="button" disabled title="לא הוגדר מספר טלפון להורה"><Phone size={14} /> אין מספר טלפון</button>}<Link className="button tiny" href={`/dashboard/garden/parents?parent=${parent.id}`}>פרופיל</Link></div>
      </article>; })}</div>}
    </section>
  );
}
  ...
  return (
    <section className="dashboard-section people-directory">
      ...
      {rows.length === 0 ? <div className="empty-state"><strong>אין אנשי צוות להצגה</strong><span>הוסיפו איש צוות דרך קליטה. עובד לא מוצג כפעיל בלי מסמכי חובה.</span><Link className="button primary" href="/dashboard/garden/onboarding">הוספת צוות</Link></div> : <div className="people-card-grid">{rows.map((member) => {
        ...
        const employmentLabel: Record<string, string> = { invited: "הוזמן/ה", pending: "ממתין/ה", pending_approval: "ממתין/ה לאישור", active: "העסקה פעילה", suspended: "מושעה", ended: "העסקה הסתיימה" };
        const employmentActive = (member.employment_status ?? member.approval_status) === "active";
        return <article className="person-card staff-profile-card" key={member.id}>
        <div className="person-card-top"><Avatar name={member.full_name} src={member.profile_photo_url ?? member.profile_image_url} size="lg" /><div><span className={employmentActive ? "pill good" : "pill bad"}>{employmentLabel[member.employment_status ?? member.approval_status] ?? "דורש בדיקה"}</span><h3>{member.full_name}</h3><p>{member.role_title ?? "צוות"} · {member.class_group ?? "כיתה לא הוגדרה"}</p></div></div>
        ...
        <div className="quick-history-cards"><span>אישור מנהלת <b>{formatDate(member.manager_approved_at)}</b></span><span>בדיקת פקח <b>{formatDate(member.inspector_verified_at)}</b></span><span>תחילת עבודה <b>{formatDate(member.employment_start_date ?? member.created_at)}</b></span></div>
