...
export function InspectorInspectionWizard({ inspections, questions }: { inspections: Inspection[]; questions: Question[] }) {
  ...
  async function submit() {
    ...
    }, () => setError("לא ניתנה הרשאת מיקום. יש לאפשר GPS כדי לשלוח ביקורת."));
  }
  ...
  return (
    <div className="inspection-wizard-shell">
      ...
      {inspection ? <section className="card action-panel"><h2>סקירה סופית</h2><p>ציון משוקלל: {score}. חריגים: {exceptions.length}. לפני השליחה תתבצע בדיקת GPS מול מיקום הגן.</p><button className="button primary large" onClick={submit}>שליחת דוח ביקורת</button></section> : null}
    </div>
  );
}
