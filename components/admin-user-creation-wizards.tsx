    ...
    try {
      const data = await postJson("/api/admin/create-garden-manager", {
        ...
        garden: {
          ...
          inspector_id: formValue(form, "assigned_inspector") || undefined, ownership_type: ownershipType, owner_role_label: ownershipType === "teacher_is_owner" ? "גננת ובעלים" : ownershipType === "separate_owner" ? "בעלים נפרד" : ownershipType === "owner_only" ? "בעלים בלבד" : "מנהלת/גננת", public_profile_enabled: formValue(form, "public_profile_enabled") === "yes", notes: formValue(form, "notes")
        },
        manager: needsManager ? { full_name: formValue(form, "manager_name"), email: formValue(form, "manager_email") || undefined, phone: formValue(form, "manager_phone") } : undefined,
        owner: needsOwner ? { full_name: formValue(form, "owner_name"), email: formValue(form, "owner_email"), phone: formValue(form, "owner_phone") } : undefined
      });
