...
export function ProfileSettingsForm({ profile, garden, roleLabel, includeGarden = false, requireProfilePhoto = false, requireGardenLogo = false }: Props) {
  ...
  return <form className="grid cols-2 dashboard-panels settings-profile-form" onSubmit={submit}><article className="card action-panel"><h2>פרטים אישיים</h2><p>{roleLabel}</p>{profileImage ? <img className="profile-preview-image" src={profileImage} alt="תמונת פרופיל" /> : <div className="empty-mini">{requireProfilePhoto ? "יש להעלות תמונת פרופיל" : "ניתן להעלות תמונת פרופיל"}</div>}<UploadImageField label={profileImage ? "החלפת תמונת פרופיל" : "העלאת תמונת פרופיל"} bucket="profile-photos" prefix="profile-images" onUploaded={setProfileImage} />{profileImage ? <button className="button secondary tiny" type="button" onClick={() => setProfileImage("")}>הסרת תמונה</button> : null}<label>שם מלא<input name="full_name" defaultValue={profile?.full_name ?? ""} required /></label><label>טלפון<input name="phone" defaultValue={profile?.phone ?? ""} /></label><label>כתובת<input name="address" defaultValue={profile?.address ?? ""} /></label><label>איש קשר לחירום<input name="emergency_contact" defaultValue={profile?.emergency_contact ?? ""} /></label></article>{includeGarden ? <article className="card action-panel"><h2>פרטי הגן</h2>{gardenLogo ? <img className="profile-preview-image" src={gardenLogo} alt="לוגו גן" /> : <div className="empty-mini">{requireGardenLogo ? "יש להעלות לוגו / תמונת גן" : "ניתן להעלות לוגו / תמונת גן"}</div>}<UploadImageField label={gardenLogo ? "החלפת לוגו / תמונת גן" : "העלאת לוגו / תמונת גן"} bucket="kindergarten-logos" prefix="garden-branding" onUploaded={setGardenLogo} />{gardenLogo ? <button className="button secondary tiny" type="button" onClick={() => setGardenLogo("")}>הסרת לוגו</button> : null}<label>שם הגן<input name="garden_name" defaultValue={garden?.name ?? ""} /></label><label>כתובת הגן<input name="garden_address" defaultValue={garden?.address ?? ""} /></label><label>טלפון הגן<input name="garden_phone" defaultValue={garden?.phone ?? ""} /></label><label>קבוצות גיל<input name="ages" defaultValue={Array.isArray(garden?.ages) ? garden.ages.join(", ") : ""} /></label><label className="wide">תיאור ציבורי<textarea name="public_description" defaultValue={garden?.public_description ?? ""} rows={4} /></label><label><input name="public_profile_enabled" type="checkbox" defaultChecked={Boolean(garden?.public_profile_enabled)} /> הצגת פרופיל ציבורי</label></article> : <article className="card action-panel"><h2>אבטחה והתראות</h2><p>כאן מרוכזים פרטי אבטחה, Passkeys והעדפות התראות. שינוי סיסמה מתבצע דרך איפוס מאובטח.</p><span className="pill good">Session פעיל</span><span className="pill">התראות מערכת</span></article>}<div className="wide profile-actions"><button className="button primary large">שמירת הגדרות</button>{message ? <span className={message.includes("נשמרו") ? "payment-action-message" : "error-text"}>{message}</span> : null}</div></form>;
}
