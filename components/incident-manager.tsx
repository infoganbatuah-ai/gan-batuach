  ...
  async function submit(formData: FormData) {
    ...
    startTransition(async () => {
      ...
      if (response.ok && body?.data) {
        setRows((current) => [{ ...body.data, children: children.find((child) => child.id === body.data.child_id) ?? null }, ...current]);
        setMessage("האירוע נשמר ונכנס לציר הטיפול.");
      } else {
        setMessage(body?.error || "לא ניתן לשמור אירוע כרגע.");
      }
    });
  }
    ...
    <section className="grid cols-2 dashboard-panels">
      <CollapsibleActionPanel title="דיווח אירוע חדש" description="אירועים קיימים מוצגים בציר. פתחו טופס רק כשצריך לתעד אירוע חדש." buttonLabel="פתיחת אירוע חדש" defaultOpen={defaultOpen || rows.length === 0 || Boolean(initialChildId)}>
        ...
        <div className="form-grid">
          ...
          <label>ילד/ה קשור/ה<select name="child_id" defaultValue={initialChildId}><option value="">לא משויך לילד</option>{children.map((child) => <option key={child.id} value={child.id}>{child.full_name}</option>)}</select></label>
        ...
        <div className="profile-actions"><button className="button primary" disabled={isPending}>{isPending ? "שומר..." : "שמירת אירוע"}</button><button className="button secondary" type="button" onClick={close}>ביטול</button></div>
      </form>}
      ...
      <article className="card action-panel">
        ...
        {rows.length === 0 ? <div className="empty-state"><strong>אין אירועים פתוחים</strong><span>אירועי בטיחות, רפואה, מצלמות ותלונות יופיעו כאן.</span></div> : <div className="timeline-list">{rows.map((incident) => <div className="timeline-item" key={incident.id}><span className={`severity-dot ${incident.severity}`} /><div><strong>{incident.title}</strong><small>{incident.incident_type} · {incident.status ?? "open"} · {incident.created_at ? new Date(incident.created_at).toLocaleString("he-IL") : ""}</small><p>{incident.description}</p>{incident.children ? <span className="selected-child-strip mini"><Avatar name={incident.children.full_name} src={incident.children.photo_url} /> {incident.children.full_name}</span> : null}</div></div>)}</div>}
      </article>
    </section>
  );
}
