...
export function GardenPickupVerificationPanel({ children, contacts, events, currentTime }: { children: Row[]; contacts: Row[]; events: Row[]; currentTime: string }) {
  ...
  async function recordPickup(formData: FormData) {
    ...
    try {
      ...
      if (!contactId) throw new Error("בחרו מורשה איסוף פעיל. חריגים דורשים טיפול נפרד.");
      ...
      <section className="grid cols-2 dashboard-panels">
        <form className="card form compact-form" action={recordPickup}>
          ...
          <div className="form-grid">
            ...
            <label>מורשה איסוף<select name="pickup_contact_id" required><option value="">בחרו מורשה פעיל</option>{contactsForChild.map((contact) => <option value={contact.id} key={contact.id}>{contact.full_name} · {relationLabels[contact.relation] ?? contact.relation}</option>)}</select></label>
          </div>
          <button className="button primary" disabled={busy || !selectedChildId || contactsForChild.length === 0}>אישור שחרור</button>
        </form>
