...
export function AdminUsersManagement({ users, auditLogs }: { users: Row[]; auditLogs: Row[] }) {
  ...
  async function action(userId: string, actionName: string) {
    ...
    try {
      await postAction(userId, actionName);
  ...
  return (
    <>
      ...
      {editing ? <section className="modal-card"><form className="card form wizard-form" onSubmit={saveEdit}><div className="section-heading"><h2>עריכת משתמש</h2><p>עדכון פרופיל נשמר ב-Supabase ונרשם בלוג ביקורת.</p></div><div className="form-grid"><label>שם מלא<input name="full_name" defaultValue={editing.full_name ?? ""} /></label><label>טלפון<input name="phone" defaultValue={editing.phone ?? ""} /></label><label>תפקיד<select name="role" defaultValue={editing.role}><option value="admin">admin</option><option value="inspector">inspector</option><option value="manager">manager</option><option value="owner">owner</option><option value="staff">staff</option><option value="parent">parent</option></select></label><label>גן משויך<input name="garden_id" defaultValue={editing.garden_id ?? ""} /></label><label>סטטוס<select name="active" defaultValue={editing.active === false ? "false" : "true"}><option value="true">פעיל</option><option value="false">לא פעיל</option></select></label><div className="wide upload-card-field"><strong>תמונת פרופיל</strong>{editing.profile_image_url ? <img className="profile-preview-image" src={editing.profile_image_url} alt="תמונת פרופיל" /> : <div className="empty-mini">ניתן להעלות תמונת פרופיל</div>}<UploadImageField label={editing.profile_image_url ? "החלפת תמונת פרופיל" : "העלאת תמונת פרופיל"} bucket="profile-photos" prefix="admin/user-profiles" onUploaded={(url) => setEditing((current) => current ? { ...current, profile_image_url: url } : current)} /><input type="hidden" name="profile_image_url" value={editing.profile_image_url ?? ""} /></div><label className="wide">הערות<textarea name="notes" rows={3} defaultValue={editing.notes ?? ""} /></label></div><div className="actions"><button className="button primary">שמירת שינויים</button><button className="button secondary" type="button" onClick={() => setEditing(null)}>ביטול</button></div></form></section> : null}
      <section className="dashboard-section people-directory">
        <div className="section-heading"><h2>פרופילי משתמשים</h2><p>הרשאות, שיוך גן והיסטוריית ביקורת. סיסמאות אינן נגישות למנהלים.</p></div>
        {rows.length === 0 ? <div className="empty-state"><strong>אין משתמשים להצגה</strong><span>כאשר משתמשים ייווצרו הם יופיעו כאן לפי הרשאה ותפקיד.</span></div> : <div className="people-card-grid admin-user-card-grid">{rows.map((user) => {
          ...
          return <article className="person-card admin-user-profile-card" key={user.id}>
            ...
            <div className="profile-badge-row">
              ...
              <span className={passwordChanged ? "pill good" : "pill warn"}>{passwordChanged ? "עודכן מצב חשבון" : "ניתן לשלוח קישור שחזור"}</span>
            </div>
            ...
            <div className="credential-box"><b>כתובת חשבון:</b> <span>{credential?.username ?? user.email ?? user.username ?? "-"}</span><strong>סיסמה אינה זמינה לצפייה.</strong></div>
