...
const statuses = ["all", "new", "assigned", "in_progress", "waiting_reporter", "escalated", "overdue", "resolved", "closed"];
...
export function AdminReportsCenter({ complaints, incidents }: { complaints: any[]; incidents: any[] }) {
  ...
  ].filter((row) => (status === "all" || row.status === status || (status === "overdue" && row.source === "complaint" && !["resolved", "closed"].includes(row.status) && [row.acknowledgement_due_at, row.response_due_at, row.resolution_due_at].some((due) => due && new Date(due).getTime() < Date.now()))) && (category === "all" || row.category === category)), [complaints, incidents, status, category]);
  ...
  async function persist(row: any, action: string) {
    ...
    setMessage("");
    const text = ["reply", "close", "create_task"].includes(action) ? window.prompt(action === "close" ? "סיכום פתרון/סגירה" : "הערה / תגובה") ?? "" : undefined;
    ...
    try {
      ...
      setMessage("הפעולה נשמרה במסד הנתונים ונרשמה בלוג.");
      router.refresh();
    } catch (error) {
