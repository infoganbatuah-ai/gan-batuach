...
export function ObserverOnboardingWizard({ packages, defaultType = "home", existingSite, createRequested = false }: { packages: any[]; defaultType?: "home" | "business"; existingSite?: any | null; createRequested?: boolean }) {
  const router = useRouter();
  const [step, setStep] = useState(1);
  const [state, setState] = useState<ActionState>({ busy: false, error: "", message: "" });
  const [newSiteConfirmed, setNewSiteConfirmed] = useState(false);
  ...
  const existingSchedule = existingSite?.monitoring_hours && typeof existingSite.monitoring_hours === "object" ? existingSite.monitoring_hours : {};
  const existingDays: string[] = Array.isArray(existingSchedule.active_days) ? (existingSchedule.active_days as unknown[]).map((item) => String(item)) : ["sun", "mon", "tue", "wed", "thu"];
  const existingTargets: string[] = Array.isArray(existingSite?.ai_features?.targets) ? (existingSite.ai_features.targets as unknown[]).map((item) => String(item)) : (defaultType === "home" ? [] : [...initialTemplate.defaultTargets]);
  const [form, setForm] = useState({ observer_site_id: existingSite?.id || "", name: existingSite?.name || "", site_type: existingSite?.site_type || defaultType, site_template: existingSite?.metadata?.site_template || initialTemplate.key, business_handles_children: Boolean(existingSite?.business_handles_children), vision_privacy_mode: existingSite?.vision_privacy_mode || "standard_consent", address_query: "", city: existingSite?.city || "", street: existingSite?.street || "", building_number: existingSite?.building_number || "", apartment_number: existingSite?.apartment_number || "", floor_kind: existingSite?.floor_kind || "floor", floor_number: existingSite?.floor_number == null ? "" : String(existingSite.floor_number), postal_code: existingSite?.postal_code || "", address_place_id: existingSite?.address_place_id || "", address_session_token: "", formatted_address: existingSite?.formatted_address || "", address_verification_status: existingSite?.address_verification_status || "unverified", camera_count: existingSite?.camera_limit || 1, branch_count: existingSite?.metadata?.branch_count || 1, active_days: existingDays, opening_time: existingSchedule.opening_time || "08:00", closing_time: existingSchedule.closing_time || "18:00", schedule_mode: existingSchedule.mode || (defaultType === "home" ? "event_only" : "business_hours"), package_id: "", monitoring_targets: existingTargets });
  ...
  async function submit() {
    ...
    try {
      const data = await postJson(
        ...
        { ...form, create_new_site: !existingSite && createRequested && newSiteConfirmed, package_id: form.package_id || null }
      );
      setState({ busy: false, error: "", message: existingSite ? "פרטי האתר עודכנו בלי לשנות את המצלמות או האירועים." : "האתר הוקם ותקופת ניסיון של 14 יום נפתחה ללא חיוב." });
  ...
  const homeMonitoringDescriptions: Record<string, string> = {
    family: "מעקב אחר פעילות המשפחה בבית",
    children: "שמירה על בטיחות הילדים",
    baby: "מעקב ושקט נפשי",
    animal: "מעקב אחר חיות המחמד",
    entry_exit: "מעקב אחר כניסות ויציאות"
  };
  ...
  return (
    <div className="do-wizard">
      ...
      {step === targetsStep ? <section className="do-panel do-form-section"><h2>{defaultType === "home" ? "מה הכי חשוב לכם?" : "מה תרצו לנטר?"}</h2><p>{defaultType === "home" ? "בחרו את הדברים החשובים בבית, כדי שנוכל להתאים עבורכם התראות חכמות ומדויקות." : "התצפיתן מתחיל מיעדי התבנית שבחרתם. אפשר לשנות דגשים בלי לבנות כללים טכניים."}</p><div className={`do-toggle-grid ${defaultType === "home" ? "do-home-priority-grid" : ""}`}>{monitoringOptions.map(([value,label]) => { const TargetIcon = defaultType === "home" ? homeMonitoringIcons[value] ?? Radar : Radar; const selected = form.monitoring_targets.includes(value); return <button type="button" className={selected ? "selected" : ""} onClick={() => toggleTarget(value)} key={value}><TargetIcon />{defaultType === "home" ? <span className="do-priority-copy"><strong>{label}</strong><small>{homeMonitoringDescriptions[value]}</small></span> : <span>{label}</span>}{defaultType === "home" ? <span className="do-option-check" aria-hidden="true">{selected ? <Check /> : null}</span> : selected ? <Check /> : null}</button>; })}</div>{defaultType === "home" ? <p className="do-onboarding-change-note">ניתן לשנות את ההעדפות בכל עת</p> : null}{defaultType === "business" ? <div className="do-notice warn"><ShieldCheck /><span>גניבה, שוד, עימות או אלימות יוצגו תמיד כחשד עם ראיות ורמת ביטחון. נדרשת החלטה אנושית; אין חיוג או פעולה אוטומטית.</span></div> : null}{defaultType === "business" && form.vision_privacy_mode === "skeleton_only" ? <div className="do-notice good"><ShieldCheck /><span>מצב פרטיות לילדים פעיל: זיהוי שלד, תנועה ודפוסים בלבד. זיהוי פנים ואודיו אינם חלק מהמסלול הזה.</span></div> : defaultType === "business" ? <div className="do-notice info"><ShieldCheck /><span>אנשים מוכרים יוצעו רק לאחר תצפיות חוזרות, וכל שמירה תחייב הסכמה מפורשת. לא מופעל זיהוי ביומטרי חי בשלב זה.</span></div> : null}</section> : null}
      {step === 4 ? <section className="do-panel do-form-section"><h2>{existingSite ? "שמירת פרטי האתר" : "בחירת חבילה והתחלת ניסיון"}</h2>{existingSite ? <div className="do-notice info"><ShieldCheck /><span>העדכון חל על האתר הקיים בלבד. מקורות המצלמה, הערוצים, האירועים, חברי האתר וההסכמות אינם מתאפסים.</span></div> : <><p>החבילה קובעת את מגבלות המצלמות והשמירה. תקופת הניסיון נמשכת 14 יום.</p><div className="do-plan-grid">{packages.filter((item) => item.package_type === form.site_type || (form.site_type === "business" && item.package_type === "enterprise")).map((item) => <label className={form.package_id === item.id ? "do-plan selected" : "do-plan"} key={item.id}><input type="radio" name="package_id" value={item.id} checked={form.package_id === item.id} onChange={() => update("package_id", item.id)} /><strong>{item.name}</strong><b>{Number(item.monthly_price || 0).toLocaleString("he-IL")} ₪</b><span>14 ימי ניסיון</span><span>עד {item.camera_limit ?? "לפי הסכם"} מצלמות</span><span>שמירת מקטעים עד {item.recording_retention_hours ?? 0} שעות</span></label>)}</div><label className="do-check"><input type="checkbox" checked={newSiteConfirmed} onChange={(event) => setNewSiteConfirmed(event.target.checked)} /><span><strong>אני מאשר/ת יצירת אתר חדש ונפרד</strong><small>האתר החדש לא יחליף אתרים או מצלמות קיימים.</small></span></label><div className="do-notice info"><ShieldCheck /><span>ספק התשלום עדיין אינו מחובר, לכן לא ייאסף כרטיס ולא יתבצע חיוב. בתקופת הפיילוט ניתן להגדיר ולבדוק מצלמות; ניטור חי ו-AI יופעלו רק לאחר חיבור ואישור הספקים.</span></div></>}</section> : null}
      <ResultMessage state={state} />
      <div className="do-wizard-actions">{step > 1 ? <button className="do-button secondary" type="button" onClick={() => setStep((value) => value - 1)}>חזרה</button> : <span />}{step < 4 ? <button className="do-button primary" type="button" disabled={!validStep} onClick={() => setStep((value) => value + 1)}>המשך <ChevronLeft /></button> : <button className="do-button primary" type="button" disabled={state.busy || (!existingSite && (!form.package_id || !newSiteConfirmed))} onClick={submit}>{state.busy ? <LoaderCircle className="do-spin" /> : null}{existingSite ? "שמירת עדכונים" : "התחלת 14 ימי ניסיון והמשך"} <ChevronLeft /></button>}</div>
    </div>
  );
}
...
export function ObserverCameraWizard({ sites, initialSiteId }: { sites: any[]; initialSiteId?: string }) {
  ...
  const additionalPairingMethods = observerCameraConnectionMethods.filter((item) => ["qr_scan", "demo"].includes(item.key));
  ...
  const stepHeading = step === 1 ? "הוספת מקור וידאו" : step === 2 ? recorderFlow ? "חיבור מקליט" : "בדיקת חיבור" : step === 3 ? "מה חשוב לך שנזהה?" : previewName;
  const stepDescription = step === 1
    ? "בחרו את סוג החיבור המתאים לכם"
    : step === 2
      ? recorderFlow ? "המקליט יתגלה ב-Gateway המקומי ויחזיר את כל הערוצים הפעילים" : "נבדוק את החיבור למצלמה"
      : step === 3
  ...
  return <div className="do-wizard do-camera-wizard">
    ...
    {step === 1 ? <section className="do-camera-mobile-first" aria-label="התחלת הוספת מצלמה">
      ...
      <small>או גללו לבחירת חיבור דרך יצרן, רשת או NVR/DVR.</small>
    </section> : null}
    {step === 1 ? <section className="do-panel do-form-section do-camera-connector-selection"><h2>בחירת מקור וידאו</h2><p>IP/ONVIF/RTSP מוסיפים מקור יחיד. NVR/DVR מוסיף מקליט אחד, ולאחר discovery יוצר את כל הערוצים שהתגלו.</p><div className="do-connector-grid do-pairing-grid do-primary-pairing-grid">{primaryPairingMethods.map((method) => { const PairingIcon = primaryPairingIcons[method.key] ?? Camera; return <button type="button" className={form.pairing_method === method.key ? "selected" : ""} onClick={() => chooseMethod(method.key)} key={method.key}><PairingIcon /><strong>{primaryPairingLabels[method.key] ?? method.label}</strong><span>{method.shortDescription}</span>{form.pairing_method === method.key ? <Check /> : <ChevronLeft />}</button>; })}</div><details className="do-additional-pairing"><summary>אפשרויות נוספות: QR ומצלמת הדמיה</summary><div className="do-connector-grid do-pairing-grid">{additionalPairingMethods.map((method) => { const PairingIcon = primaryPairingIcons[method.key] ?? Camera; return <button type="button" className={form.pairing_method === method.key ? "selected" : ""} onClick={() => chooseMethod(method.key)} key={method.key}><PairingIcon /><strong>{method.label}</strong><span>{method.shortDescription}</span><b className="do-badge info">{method.badge}</b>{form.pairing_method === method.key ? <Check /> : null}</button>; })}</div></details><div className="do-notice info"><ShieldCheck /><span>פרטי חיבור רגישים נשמרים רק ברכיב המקומי או בכספת השרת, ולא בדפדפן.</span></div></section> : null}
    {step === 2 && recorderFlow ? <section className="do-panel do-form-section do-camera-test-step do-recorder-gateway-step">
      <div className="do-section-head"><div><h2>חיבור DVR / NVR</h2><p>מוסיפים מקליט אחד. הרכיב המקומי יגלה את הערוצים בפועל ויוסיף אותם יחד לאתר הנבחר.</p></div><span className="do-badge info">מקליט רב־ערוצי</span></div>
      ...
      <div className="do-notice info"><HardDrive /><span>לא יוצרים כאן מצלמה בודדת ולא מזינים כתובת, שם משתמש או סיסמת DVR בדשבורד. הפרטים, אם נדרשים, מוזנים פעם אחת ברכיב המקומי בלבד.</span></div>
      <ObserverGatewayPairing observerSiteId={form.observer_site_id} />
      <div className="do-connection-check-result"><span><Check /></span><div><strong>לאחר CONNECT יתגלה מספר הערוצים האמיתי</strong><small>המסך יחזור לרשימת המצלמות רק אחרי discovery ומיפוי מאומת. אין PTZ, סירנה או שינוי הגדרות במקליט.</small></div><b>קריאה בלבד</b></div>
    </section> : null}
    {step === 2 && !recorderFlow ? <section className="do-panel do-form-section do-camera-test-step"><div className="do-section-head"><div><h2>בדיקת חיבור</h2><p>נבדוק את החיבור למצלמה בלי לחשוף סיסמה בדפדפן.</p></div><span className={selectedMethod.requiresGateway ? "do-badge warn" : "do-badge info"}>{selectedMethod.requiresGateway ? "דורש רכיב מקומי" : "בדיקה מאובטחת"}</span></div>{requiresLocalDvrGateway ? <div className="do-notice warn"><HardDrive /><span><strong>חיבור מקליט מתבצע דרך רכיב מקומי מזוהה.</strong> במסלול הרגיל מתקינים/פותחים את רכיב החיבור, מאמתים את החשבון פעם אחת או סורקים QR, והדשבורד מזהה שהוא מחובר לאתר הזה. קוד pairing קצר־חיים נשאר fallback בלבד למכשיר אחר, headless או recovery. פרטי DVR מוזנים רק ברכיב המקומי ולא נשמרים בדשבורד.</span></div> : null}<div className="do-camera-connection-form"><div className="do-form-grid"><label className="do-field"><span>אתר</span><select value={form.observer_site_id} onChange={(event) => update("observer_site_id", event.target.value)}>{sites.map((site) => <option key={site.id} value={site.id}>{site.name}</option>)}</select></label><label className="do-field"><span>שם המצלמה</span><input value={form.display_name} onChange={(event) => update("display_name", event.target.value)} placeholder="מצלמת כניסה" /></label><label className="do-field"><span>מיקום</span><input value={form.location_label} onChange={(event) => update("location_label", event.target.value)} placeholder="כניסה ראשית" /></label><label className="do-field"><span>יצרן / מתאם</span><select value={form.connector_provider} onChange={(event) => update("connector_provider", event.target.value)}>{observerCameraManufacturers.map((manufacturer) => <option key={manufacturer.value} value={manufacturer.value}>{manufacturer.label}</option>)}</select></label></div><dl className="do-connection-facts"><div><dt>סוג חיבור</dt><dd>{primaryPairingLabels[selectedMethod.key] ?? selectedMethod.label}</dd></div><div><dt>מתאם</dt><dd>{selectedMethod.requiresGateway ? "רכיב מקומי מאומת" : "ONVIF / ספק מאושר"}</dd></div><div><dt>סטטוס</dt><dd>{requiresLocalDvrGateway ? "ממתין לזיהוי רכיב או fallback pairing" : "מוכן לבדיקה"}</dd></div></dl></div><div className="do-camera-test-preview"><ObserverCameraMedia name={previewName} scene={previewMode === "home" ? "home-entry" : "business-entry"} mode={previewMode} status="readiness" sourceMode={form.connector_type === "demo" ? "demo" : "readiness"} /><div className="do-camera-preview-controls" aria-label="בקרות תצוגה במצב מוכנות"><span>תצוגה מקדימה</span><button type="button" disabled aria-label="השהיה"><Camera /></button><button type="button" disabled aria-label="שמע"><Mic /></button><button type="button" disabled aria-label="צילום"><Image /></button></div></div><div className="do-connection-check-result"><span><Check /></span><div><strong>{requiresLocalDvrGateway ? "האתר נבחר למיפוי המקומי" : "הפרטים מוכנים לבדיקה"}</strong><small>{requiresLocalDvrGateway ? "אין להזין כאן כתובת או סיסמת DVR. הרכיב המקומי מבצע discovery ומחזיר רק מצב ערוצים מאומת." : "החיבור ייבדק בפועל לאחר חיבור רכיב מקומי או ספק מאושר."}</small></div><b>{selectedMethod.requiresGateway ? "ממתין לרכיב מקומי" : "ממתין לספק"}</b></div>{form.pairing_method === "qr_scan" ? <div className="do-qr-capture"><label className="do-button secondary"><QrCode /> צילום או בחירת קוד QR<input type="file" accept="image/*" capture="environment" onChange={(event) => void scanQrImage(event.target.files?.[0])} /></label>{scanState ? <p>{scanState}</p> : <span>הסריקה מתבצעת במכשיר. התמונה ותוכן הקוד אינם נשמרים.</span>}</div> : null}<details className="do-camera-test-help"><summary>הוראות חיבור מפורטות</summary><div className="do-pairing-instructions"><div><strong>מה צריך להכין</strong><ul>{selectedMethod.requiredItems.map((item) => <li key={item}>{item}</li>)}</ul></div><ol>{selectedMethod.steps.map((item) => <li key={item}>{item}</li>)}</ol></div></details></section> : null}
...
export function ObserverKnownPersonForm({ siteId, cameras }: { siteId: string; cameras: Array<{ id: string; display_name?: string | null; location_label?: string | null }> }) {
  const router = useRouter(); const [state, setState] = useState<ActionState>({ busy: false, error: "", message: "" });
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const data = new FormData(event.currentTarget); setState({ busy: true, error: "", message: "" }); try { const result = await postJson("/api/digital-observer/known-people", { action: "create", observer_site_id: siteId, display_name: data.get("display_name"), relationship_label: data.get("relationship_label"), access_class: data.get("access_class"), consent_confirmed: data.get("consent") === "on", notify_on_detection: data.get("notify") === "on", camera_source_ids: data.getAll("camera_source_ids").map(String) }); setState({ busy: false, error: "", message: result.message }); event.currentTarget.reset(); router.refresh(); } catch (error) { setState({ busy: false, error: error instanceof Error ? error.message : "לא ניתן לשמור", message: "" }); } }
  return <form className="do-panel do-form-section" onSubmit={submit}><h2>הוספת אדם מוכר</h2><p>המערכת לא מפעילה זיהוי פנים נסתר. ההגדרה חלה רק על המצלמות שבוחרים כאן; תמונה ועיבוד ביומטרי יתווספו רק לאחר הסכמה וחיבור מאושר.</p><div className="do-form-grid"><label className="do-field"><span>שם</span><input name="display_name" required minLength={2} /></label><label className="do-field"><span>קשר</span><input name="relationship_label" placeholder="בן משפחה / עובד / בעל גישה" /></label><label className="do-field"><span>סוג הרשאה</span><select name="access_class" defaultValue="authorized_visitor"><option value="household_resident">דייר/ת הבית</option><option value="authorized_visitor">מאושר/ת כניסה</option><option value="service_provider">נותן/ת שירות</option><option value="other">אחר</option></select></label></div><fieldset className="do-field"><legend>מצלמות מורשות לתצפיות על האדם</legend><div className="do-channel-grid">{cameras.map((camera) => <label key={camera.id}><input name="camera_source_ids" type="checkbox" value={camera.id} /><Camera /><span><strong>{camera.display_name || "מצלמה"}</strong><small>{camera.location_label || "מקור מחובר"}</small></span></label>)}</div></fieldset><label className="do-check"><input name="consent" type="checkbox" required /><span>יש בידי הסכמה מפורשת של האדם להגדרה זו ולמצלמות שנבחרו</span></label><label className="do-check"><input name="notify" type="checkbox" /><span>להכין התראה כאשר הזיהוי יהיה פעיל</span></label><button className="do-button primary" type="submit" disabled={state.busy || !cameras.length}>שמירה במצב מוכנות</button>{!cameras.length ? <p role="alert">נדרש לפחות מקור מצלמה מחובר לפני יצירת פרופיל.</p> : null}<ResultMessage state={state} /></form>;
}

export function ObserverSettingsForm({ siteId, schedule, channels, consent = {} }: { siteId: string; schedule?: any; channels: any[]; consent?: Record<string, unknown> }) {
  const router = useRouter(); const [state, setState] = useState<ActionState>({ busy: false, error: "", message: "" });
  const enabled = (channel: string) => Boolean(channels.find((item) => item.channel === channel)?.enabled);
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const data = new FormData(event.currentTarget); setState({ busy: true, error: "", message: "" }); try { const result = await postJson("/api/digital-observer/settings", { observer_site_id: siteId, schedule_mode: data.get("schedule_mode"), quiet_start: data.get("quiet_start") || undefined, quiet_end: data.get("quiet_end") || undefined, in_app: data.get("in_app") === "on", email: data.get("email") === "on", push: data.get("push") === "on", sms: data.get("sms") === "on", whatsapp: data.get("whatsapp") === "on", monitoring_consent: data.get("monitoring_consent") === "on", safe_action_consent: data.get("safe_action_consent") === "on", model_improvement_consent: data.get("model_improvement_consent") === "on" }); setState({ busy: false, error: "", message: result.message }); router.refresh(); } catch (error) { setState({ busy: false, error: error instanceof Error ? error.message : "לא ניתן לשמור", message: "" }); } }
