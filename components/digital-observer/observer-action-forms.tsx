...
export function ObserverOnboardingWizard({ packages, defaultType = "home" }: { packages: any[]; defaultType?: "home" | "business" }) {
  ...
  const [form, setForm] = useState({ name: "", site_type: defaultType, business_handles_children: false, vision_privacy_mode: "standard_consent", ...emptyAddress, camera_count: 1, schedule_mode: defaultType === "home" ? "event_only" : "business_hours", package_id: "", monitoring_targets: ["person", "unknown_person", "entry_exit", "vehicle", "animal", "distress", "camera_obstruction", "door_left_open"] });
  ...
  return (
    <div className="do-wizard">
      ...
      {step === 3 ? <section className="do-panel do-form-section"><h2>מה תרצו לראות ראשון?</h2><p>התצפיתן כבר מתחיל עם תחומי הלמידה המרכזיים. אפשר להדגיש נושאים, אבל אין צורך לבנות כללים טכניים כדי שיתחיל ללמוד את המקום.</p><div className="do-toggle-grid">{[["person","אדם"],["unknown_person","אדם לא מוכר"],["animal","בעל חיים"],["entry_exit","כניסה ויציאה"],["vehicle","רכב וחניה"],["distress","סימני מצוקה"],["after_hours","פעילות מחוץ לשעות"],["camera_obstruction","מצלמה מכוסה"],["restricted_area","אזור מוגבל"],["door_left_open","דלת שנשארה פתוחה"]].map(([value,label]) => <button type="button" className={form.monitoring_targets.includes(value) ? "selected" : ""} onClick={() => toggleTarget(value)} key={value}><Radar /><span>{label}</span>{form.monitoring_targets.includes(value) ? <Check /> : null}</button>)}</div>{form.vision_privacy_mode === "skeleton_only" ? <div className="do-notice good"><ShieldCheck /><span>מצב פרטיות לילדים פעיל: זיהוי שלד, תנועה ודפוסים בלבד. זיהוי פנים ואודיו אינם חלק מהמסלול הזה.</span></div> : <div className="do-notice warn"><ShieldCheck /><span>אנשים מוכרים יוצעו רק לאחר תצפיות חוזרות, וכל שמירה תחייב הסכמה מפורשת. לא מופעל זיהוי ביומטרי חי בשלב זה.</span></div>}</section> : null}
