...
export function ObserverSourceCoverage({ siteId, cameras, signals, available, limited }: {
  siteId: string; cameras: Record<string, any>[]; signals: Record<string, any>[]; available: boolean; limited: boolean;
}) {
  const rows = observerSourceCoverage(siteId, cameras, signals);
  ...
  return <section className="do-source-coverage" aria-label="כיסוי לפי מצלמה">
    ...
    <p className="do-source-coverage-notice">{!available ? "דיווחי האירועים לא נטענו כרגע." : limited ? "רשימת הדיווחים שנטענה חלקית; הספירות אינן סיכום מלא." : "הספירות מתייחסות לדיווחים שנשמרו, לא לסריקת הקלטות."} כיסוי סבבי הניתוח טרם אומת; חוסר דיווחים אינו מעיד שלא הייתה פעילות.</p>
    <details><summary>פירוט הכיסוי של {rows.length} המקורות</summary><div className="do-source-coverage-list">{rows.map(row => <article key={row.id}>
      ...
      <span className="do-source-coverage-time"><Clock3 />{available && row.lastRecordAt ? `דיווח אחרון: ${formatObserverDate(row.lastRecordAt)}` : "אין דיווח אחרון זמין"}<small>ניתוח אחרון: לא דווח</small></span>
    </article>)}</div></details>
  </section>;
}
