...
export function ObserverCameraPresence({ active = false }: { active?: boolean }) {
  return <span className={`do-camera-observer-presence ${active ? "active" : ""}`} title={active ? "המקור מדווח כמחובר לווידאו. מצב ניתוח התצפיתן טרם אומת מתג זה." : "חיבור הווידאו אינו מאומת. תג זה אינו קובע את מצב מנוע הניתוח."}>
    <Image src="/assets/digital-observer/observer-robot-v1.png" alt="" width={84} height={84} />
    <i />
    <small>{active ? "חיבור וידאו מדווח" : "חיבור לא מאומת"}</small>
  </span>;
}
