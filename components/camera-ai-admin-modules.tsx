...
export function CameraAdminManager({ cameras, gardens, gatewayConnected }: { cameras: CameraRow[]; gardens: Garden[]; gatewayConnected: boolean }) {
  ...
  async function save(event: FormEvent<HTMLFormElement>, close?: () => void) {
    ...
    try {
      ...
      const sourceType = systemType === "sample_hls" ? "Sample HLS" : systemType === "onvif" ? "ONVIF" : systemType === "ip_camera" ? "IP Camera" : systemType === "dvr_nvr" ? "DVR/NVR" : "RTSP";
  ...
  return <><div className={gatewayConnected ? "success-screen" : "gateway-setup-state"}><strong>{gatewayConnected ? "Gateway וידאו מחובר" : "Gateway וידאו עדיין לא מחובר"}</strong><p>הפלטפורמה תומכת ברישום DVR/NVR/IP/RTSP/ONVIF ובניגון מאובטח דרך Gateway. טופס ההוספה נפתח רק כשצריך כדי לשמור על מסך נקי.</p></div><section className="grid cols-4 dashboard-panels"><article className="card metric-card"><span>מחוברות</span><strong>{health.online}</strong></article><article className="card metric-card"><span>אזהרות</span><strong>{health.warning + health.pending}</strong></article><article className="card metric-card"><span>לא מחוברות</span><strong>{health.offline}</strong></article><article className="card metric-card"><span>מושבתות</span><strong>{health.disabled}</strong></article></section>{message ? <div className="success-banner">{message}</div> : null}{error ? <div className="error-banner">{error}</div> : null}<CollapsibleActionPanel title={hasCameras ? "הוספת מצלמה חדשה" : "עדיין לא נוספו מצלמות"} description={hasCameras ? "המצלמות הקיימות מוצגות למטה. פתחו את הטופס רק כשצריך להוסיף מקור חדש." : "אפשר להוסיף מצלמה ראשונה ולשמור אותה כממתינה ל-Gateway או עם Sample HLS לבדיקה."} buttonLabel={hasCameras ? "הוספת מצלמה חדשה" : "הוספת מצלמה ראשונה"} defaultOpen={!hasCameras}>{({ close }) => <section className="grid cols-2 dashboard-panels"><form className="card form wizard-form" onSubmit={(event) => save(event, close)}><h2>הוספת מצלמה</h2><div className="form-grid"><label>גן<select name="garden_id" required><option value="">בחר גן</option>{gardens.map((g) => <option key={g.id} value={g.id}>{g.name} · {g.city}</option>)}</select></label><label>שם מצלמה<input name="name" required /></label><label>סוג מערכת<select name="system_type"><option value="dvr_nvr">מערכת מצלמות DVR / NVR</option><option value="ip_camera">מצלמת IP</option><option value="onvif">מצלמת ONVIF</option><option value="manual_rtsp">חיבור ידני</option><option value="sample_hls">Sample HLS לבדיקה</option></select></label><label>איכות שידור<select name="stream_quality"><option value="sub">רגילה / קלה</option><option value="main">גבוהה</option></select></label><label>כתובת המערכת<input name="connection_host" placeholder="IP או דומיין" /></label><label>כתובת חיבור ידנית<input name="manual_rtsp_url" placeholder="רק אם הטכנאי סיפק כתובת מוכנה" /></label><label>פורט<input name="connection_port" placeholder="554" /></label><label>שם משתמש<input name="username" /></label><label>סיסמה<input name="password" type="password" autoComplete="new-password" /><small>הסיסמה מוצפנת בשרת ולא מוצגת שוב לאחר שמירה.</small></label><label>מספר ערוץ<input name="connection_channel" type="number" min="1" placeholder="1" /></label><label>Sample HLS לבדיקה<input name="hls_playback_url" placeholder="https://.../index.m3u8" /></label><label>כיתה/אזור<input name="area" required /></label><label>קבוצת גיל<input name="age_group" /></label><label>כיתה<input name="class_group" /></label><label>שעות צפייה<input name="viewing_hours" placeholder="08:00-12:00" /></label><label>ימי שמירה עתידיים<input name="retention_days" type="number" min="0" placeholder="לא פעיל עדיין" /></label><label>מדיניות ארכיון<input name="archive_policy" placeholder="למשל: מחיקה אחרי 14 יום" /></label><label><input name="recording_enabled" type="checkbox" /> הכנה להקלטה עתידית</label><label><input name="parent_view_allowed" type="checkbox" /> צפיית הורים מותרת</label></div><div className="profile-actions"><button className="button secondary" type="button" onClick={(event) => { const form = event.currentTarget.form; if (form) void testDraft({ preventDefault() {}, currentTarget: form } as any); }}>בדיקת חיבור</button><button className="button primary">שמירת מצלמה</button><button className="button secondary" type="button" onClick={close}>ביטול</button></div></form><article className="card action-panel"><h2>שלבי אשף</h2><div className="stepper expanded">{["בחר גן","בחר מקור","פרטי חיבור","שיוך","בדיקה","שמירה","הרשאות","צפייה"].map((s,i)=><span key={s}><b>{i+1}</b>{s}</span>)}</div></article></section>}</CollapsibleActionPanel><section className="dashboard-section"><div className="section-heading"><h2>תצפיתן דיגיטלי - צפייה במצלמות</h2><p>סטטוס, גן, אזור, הרשאות וצפייה מאובטחת ללא חשיפת RTSP.</p></div>{rows.length===0?<div className="empty-state"><strong>אין מצלמות עדיין</strong><span>לחצו על “הוספת מצלמה ראשונה” כדי להתחיל בצורה מסודרת.</span></div>:<div className="camera-playback-grid">{rows.map((cam)=><div key={cam.id} className="camera-operation-stack"><CameraPlaybackCard camera={cam} /><div className="camera-admin-verification"><span>סוג מערכת: {systemTypeLabels[cam.system_type] ?? cam.system_type ?? cam.source_type ?? "מערכת מצלמות"}</span><span>בדיקה אחרונה: {cam.last_test_message ?? "טרם נבדקה"}</span><span>Gateway: {cam.gateway_registration_status ?? cam.connection_method ?? "ממתין"}</span><span>פרטי חיבור: {cam.masked_connection_summary?.password_present ? "סיסמה שמורה ומוצפנת" : "ללא סיסמה שמורה"}</span></div><div className="profile-actions"><button className="button secondary tiny" type="button" onClick={() => cameraAction(cam, "test_connection")}>בדיקת חיבור</button><button className="button secondary tiny" type="button" onClick={() => cameraAction(cam, cam.active === false ? "enable" : "disable")}>{cam.active === false ? "הפעלה" : "השבתה"}</button><button className="button secondary tiny" type="button" onClick={() => cameraAction(cam, "mark_offline")}>סימון תקלה</button></div></div>)}</div>}</section></>;
}
