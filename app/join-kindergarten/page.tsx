      ...
      <main>
        ...
        <section className="section wizard-layout">
          ...
          <form action={createGardenLead} className="card form wizard-form premium-step-form">
            ...
            <details className="accordion-step" open><summary><strong>1. פרטי מנהלת / בעלים</strong><span>איש קשר ראשי להמשך</span></summary><div className="form-grid"><label>שם בעלים *<input name="owner_name" required /></label><label>שם מנהל/גננת<input name="manager_name" /></label><label>טלפון *<input name="phone" required /></label><label>מייל<input name="email" type="email" /></label></div></details>
            ...
            <details className="accordion-step"><summary><strong>4. כתובת, קשר ותפעול</strong><span>מצלמות, מטבח וסטטוס מוכנות</span></summary><div className="form-grid"><label>האם קיימות מצלמות?<select name="camera_status"><option value="no">לא</option><option value="dvr_nvr">כן, DVR/NVR</option><option value="ip">כן, מצלמות IP</option><option value="unknown">צריך בדיקת חיבור</option></select></label><label>מטבח / אוכל<select name="food_kitchen"><option>יש מטבח</option><option>ספק אוכל חיצוני</option><option>אין מטבח</option></select></label><label>סטטוס מסמכים<select name="documents_status"><option>לא הועלו עדיין</option><option>קיימים חלקית</option><option>קיימים ומוכנים לבדיקה</option></select></label><label className="wide">הערות<textarea name="notes" rows={4} placeholder="מועדי פעילות, צרכים, מצב פיקוח וכל פרט חשוב" /></label></div></details>
