...
function logFinanceError(label: string, gardenId: string, error: any) {
  console.error("[garden-finance] query failed", {
    label,
    garden_id: gardenId,
...
export default async function GardenFinancePage({ searchParams }: { searchParams: Promise<{ filter?: string }> }) {
  ...
  const debtTotal = allChildren.reduce((sum, child) => sum + Number(child.debt_amount ?? 0), 0);
