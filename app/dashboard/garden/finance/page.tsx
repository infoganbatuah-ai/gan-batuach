...
export default async function GardenFinancePage() {
  ...
  const [childrenRes, historyRes, feeGroupsRes, monthHistoryRes, yearHistoryRes] = await Promise.all([
    supabase.from("children" as any).select("id, full_name, photo_url, age_group, classroom, payment_group_id, monthly_fee, custom_monthly_fee, arrangement_notes, arrangement_valid_until, payment_status, payments_paused, paused_reason, debt_amount, debt_notes, last_payment_date, next_payment_due, valid_until, payment_notes, last_amount_paid, last_payment_method").eq("garden_id", gardenId).order("full_name"),
    supabase.from("child_payment_history" as any).select("id, child_id, amount, amount_paid, action, payment_status, paid_at, valid_from, valid_until, payment_method, transaction_type, notes, children(full_name)").eq("garden_id", gardenId).order("created_at", { ascending: false }).limit(40),
