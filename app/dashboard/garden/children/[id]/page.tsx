  ...
  return (
    <DashboardShell role={profile.role === "owner" ? "owner" : "manager"} title="כרטיס ילד">
      <section className="dashboard-section printable-report">
        ...
        <div className="profile-actions"><Link className="button secondary" href="/dashboard/garden/children">חזרה לילדים</Link><Link className="button primary" href={`/dashboard/garden/children/${child.id}/timeline`}>ציר בטיחות</Link><Link className="button secondary" href={`/dashboard/garden/messages?childId=${child.id}`}>הודעה להורה</Link><PrintButton label="הדפסת כרטיס ילד / PDF" /></div>
