...
export default async function GardenMessagesPage() {
  ...
  const [parentsRes, staffRes, inspectorsRes, childrenRes, messagesRes] = await Promise.all([
    ...
    supabase.from("children" as any).select("id, full_name").eq("garden_id", gardenId).order("full_name"),
  ...
  return (
    <DashboardShell role="manager" title="הודעות">
      ...
      <InternalMessagingCenter gardenId={gardenId} recipients={recipients} linkedChildren={(childrenRes.data ?? []) as any[]} messages={(messagesRes.data ?? []) as any[]} />
    </DashboardShell>
  );
}
