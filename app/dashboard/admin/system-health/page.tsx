...
export default async function AdminSystemHealthPage() {
  ...
  return <DashboardShell role="admin" title="בריאות מערכת"><div className="dashboard-hero-card admin-hero-card"><div><p className="eyebrow">System Health</p><h1>מה חסר כדי שהמערכת תהיה מוכנה להפעלה מלאה?</h1><p>גנים ללא מנהלת/פקח, ביקורת ראשונה חסרה, צוות ללא מסמכים, ילדים עם מידע רפואי חסר, מצלמות ו־AI.</p></div><span className={issues.length ? "pill bad" : "pill good"}>{issues.length ? `${issues.length} בעיות` : "תקין"}</span></div><AdminDataError message={result.error ?? result.data.queryError} /><section className="dashboard-section">{issues.length === 0 ? <div className="empty-state"><CheckCircle2 /><strong>אין בעיות מערכת פתוחות</strong><span>כאשר חסר Setup הכרחי, הוא יופיע כאן עם פעולה מומלצת.</span></div> : <div className="procedure-list">{issues.map((issue, index) => <article className="card procedure-card" key={index}><div><span className={issue.severity === "critical" || issue.severity === "high" ? "pill bad" : "pill warn"}><AlertTriangle size={14} /> {issue.severity}</span><h3>{issue.title}</h3><p>{issue.garden ?? "כללי"}</p><small>פעולה מומלצת: {issue.fix}</small></div><div className="procedure-meta"><Link className="button secondary" href={issue.href}><Activity size={15} /> טיפול</Link></div></article>)}</div>}</section></DashboardShell>;
}
