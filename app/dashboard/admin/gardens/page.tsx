...
export default async function Page() {
  ...
  return <AdminAppFrame profile={profile} activeHref="/dashboard/admin/kindergarten-applications" title="גנים" subtitle="ספריית גנים, סטטוס תפעולי וכניסה לפרופיל מלא." badge={`${rows.length} גנים`}><PremiumDashboardHero eyebrow="גנים" title="ספריית גנים נקייה." subtitle="סטטוס, מנהלת וכניסה לפרופיל מלא." actions={<div className="actions"><Link className="button primary" href="/dashboard/admin/leads">קליטת גן חדש</Link><Link className="button secondary" href="/dashboard/admin/kindergartens">ספרייה מורחבת</Link></div>} badge={`${rows.length} גנים`} badgeTone="good" /><AdminDataError message={result.error ?? result.data.queryError} /><section className="filter-bar"><input placeholder="חיפוש לפי שם גן / עיר" /><select><option>כל הסטטוסים</option><option>active</option><option>pending</option><option>blocked</option></select></section><section className="dashboard-section">{rows.length === 0 ? <div className="empty-state"><strong>אין גנים להצגה</strong><span>גן חדש יופיע כאן אחרי אישור וקליטה.</span></div> : <div className="procedure-list">{rows.map((garden) => <article className="card procedure-card" key={garden.id}><div><span className={garden.safe_status === "safe" ? "pill good" : "pill warn"}>{garden.safe_status ?? "pending"}</span><h3>{garden.name}</h3><p>{garden.city} · {garden.address ?? "ללא כתובת"}</p><small>מנהלת: {garden.managers?.full_name ?? "-"} · בעלים: {garden.owners?.full_name ?? "-"} · ילדים: {garden.current_children_count ?? 0}/{garden.children_capacity ?? 0}</small></div><div className="procedure-meta"><Link className="button secondary" href={`/dashboard/admin/gardens/${garden.id}`}>פרופיל גן</Link><Link className="button" href={`/dashboard/admin/users/new-kindergarten?gardenId=${garden.id}`}>עריכה</Link></div></article>)}</div>}</section></AdminAppFrame>;
}
