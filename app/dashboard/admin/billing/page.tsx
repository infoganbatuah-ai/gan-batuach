...
function monthlyValue(subscription: any) {
  ...
  const cycle = String(subscription.billing_cycle ?? subscription.plan_type ?? "monthly");
    ...
    <DashboardShell role="admin" title="חיוב והכנסות">
      <div className="commercial-dashboard">
        <PremiumDashboardHero
          ...
          title="מנוע הכנסות, מנויים וחשבוניות"
          subtitle="מרכז אחד למנויי גנים, ספקי תשלום, כשלי גבייה, חידושים, חשבוניות, החזרים ותחזית הכנסות."
          badge={money(mrr)}
        ...
        <section className="grid cols-4 action-grid">
          <ActionCard title="תוכניות מנוי" text="חודשי, שנתי, פיילוט ורשתות" href="/dashboard/admin/billing#plans" icon={CreditCard} tone="good" />
                ...
                {data.subscriptions.slice(0, 12).map((subscription) => (
                  <article className="list-item" key={subscription.id}>
                    <div>
                      ...
                      <span>{subscription.subscription_plans?.name ?? subscription.plan_type} · {subscription.billing_cycle ?? "monthly"} · חידוש {dateText(subscription.renewal_date)}</span>
        ...
        <section className="grid cols-3 dashboard-panels" id="plans">
          <CleanSection title="תוכניות" subtitle="חודשי, שנתי, פיילוט, קידום ורשתות.">
            <div className="stack-list">
              {data.plans.map((plan) => (
                <article className="list-item" key={plan.id}>
                  <div>
                    <strong>{plan.name}</strong>
                    <span>{money(plan.monthly_price ?? plan.price_amount, plan.currency)} חודשי · {money(plan.annual_price, plan.currency)} שנתי</span>
