...
import { AlertTriangle, BellRing, Bot, Camera, ClipboardCheck, Download, FileWarning, MapPinned, Settings, ShieldAlert, UserCheck, UsersRound } from "lucide-react";
...
export default async function AdminDashboard() {
  ...
  const result = await safeAdminData("admin dashboard", async () => {
    ...
    const [gardens, leads, inspectors, violations, complaints, cameras, notifications, incidents, gardenList] = await Promise.all([
    ...
    return { gardens, leads, inspectors, violations, complaints, cameras, notifications, incidents, gardenList: (gardenList.data ?? []) as any[], queryError: gardenList.error ? "לא ניתן לטעון את הנתונים כרגע" : null };
  }, { gardens: 0, leads: 0, inspectors: 0, violations: 0, complaints: 0, cameras: 0, notifications: 0, incidents: 0, gardenList: [] as any[], queryError: null as string | null });
  ...
  return (
    <DashboardShell role="admin" title="מרכז שליטה ארצי">
      ...
      <div className="grid cols-4 dashboard-kpis"><StatCard label="גנים" value={data.gardens} tone="good" /><StatCard label="פקחים" value={data.inspectors} /><StatCard label="לידים" value={data.leads} tone="warn" /><StatCard label="אירועים" value={data.incidents} tone="bad" /></div>
