...
const parentActions = [
  ...
  { href: "/dashboard/parent/documents", label: "מסמכים ואישורים", icon: ShieldCheck, text: "מה שחסר, מאושר או ממתין להשלמה." },
...
export default async function ParentDashboard() {
  ...
  return (
    <DashboardShell role="parent" title="אזור הורים">
      ...
      <section className="dashboard-section role-priority-section"><div className="section-heading"><h2>מה אפשר לעשות עכשיו?</h2><p>הפעולות הכי שימושיות להורה: הודעה לגן, מסמכים, מצלמות ועדכונים יומיים.</p></div><div className="quick-actions-grid role-action-grid">{parentActions.map((action) => <Link className="quick-action" href={action.href} key={action.label}><action.icon /><strong>{action.label}</strong><span>{action.text}</span></Link>)}</div></section>
