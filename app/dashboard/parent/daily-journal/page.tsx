...
export default async function ParentDailyJournalPage() {
  ...
  return (
    <DashboardShell role="parent" title="יומן יומי">
      ...
      <section className="dashboard-section">{(journalsRes.data ?? []).length === 0 ? <div className="empty-state"><strong>אין עדכונים יומיים עדיין</strong><span>כאשר הגן יעדכן יומן יומי, הוא יופיע כאן עם תמונות והערות.</span></div> : <div className="journal-grid">{(journalsRes.data ?? []).map((journal: any) => <article className="card journal-card" key={journal.id}><div className="selected-child-strip"><Avatar name={journal.children?.full_name} src={journal.children?.photo_url} /><strong>{journal.children?.full_name}</strong><span>{new Date(journal.journal_date).toLocaleDateString("he-IL")}</span></div><div className="journal-facts"><span><Utensils size={15} /> {(journal.meals ?? []).map((meal: any) => meal.note).filter(Boolean).join(", ") || "ארוחות לא עודכנו"}</span><span><Moon size={15} /> {journal.sleep_summary ?? "שינה לא עודכנה"}</span><span>מצב רוח: {journal.mood ?? "לא עודכן"}</span><span>שירותים: {journal.bathroom ?? "לא עודכן"}</span></div>{journal.medicine ? <p className="pill warn">תרופה: {journal.medicine}</p> : null}{journal.incidents ? <p className="pill bad">אירוע: {journal.incidents}</p> : null}<p>{journal.notes_to_parents ?? "אין הערה נוספת."}</p>{Array.isArray(journal.photo_urls) && journal.photo_urls.length ? <div className="photo-strip">{journal.photo_urls.map((url: string) => <a href={url} key={url}><Camera size={15} /> תמונה</a>)}</div> : null}<small>חתימת צוות: {journal.staff_signature ?? "לא צוינה"}</small></article>)}</div>}</section>
    </DashboardShell>
  );
}
