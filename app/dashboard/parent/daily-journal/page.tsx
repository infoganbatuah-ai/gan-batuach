      ...
      <ParentAppFrame active="calendar" profileName={profile.full_name} avatarUrl={(profile as any).profile_image_url ?? null}>
        ...
        <ParentSection title="עדכונים מהגן" subtitle="המידע מוצג בשפה פשוטה ומרגיעה." action={<span className="pill good"><BookHeart size={15} /> עדכונים מאושרים</span>}>
          {(journalsRes.data ?? []).length === 0 ? <ParentEmptyState title="אין עדכונים יומיים עדיין" text="כאשר הגן יעדכן יומן יומי, הוא יופיע כאן עם תמונות והערות." /> : (
            <div className="parent-journal-feed">
              {(journalsRes.data ?? []).map((journal: any) => (
                <article className="parent-journal-card" key={journal.id}>
                  <div className="selected-child-strip"><Avatar name={journal.children?.full_name} src={journal.children?.photo_url} /><strong>{journal.children?.full_name}</strong><span>{new Date(journal.journal_date).toLocaleDateString("he-IL")}</span></div>
                  <p className="parent-daily-summary">{journal.notes_to_parents ?? `${journal.children?.full_name ?? "הילד/ה"} קיבל/ה עדכון יומי מהגן.`}</p>
                  <div className="journal-facts"><span><Utensils size={15} /> {(journal.meals ?? []).map((meal: any) => meal.note).filter(Boolean).join(", ") || "ארוחות לא עודכנו"}</span><span><Moon size={15} /> {journal.sleep_summary ?? "שינה לא עודכנה"}</span><span>מצב רוח: {journal.mood ?? "לא עודכן"}</span><span>שירותים: {journal.bathroom ?? "לא עודכן"}</span></div>
                  {journal.medicine ? <p className="pill warn">תרופה: {journal.medicine}</p> : null}
                  {journal.incidents ? <p className="pill bad">אירוע לטיפול: {journal.incidents}</p> : null}
                  {Array.isArray(journal.photo_urls) && journal.photo_urls.length ? <div className="photo-strip">{journal.photo_urls.map((url: string) => <a href={url} key={url}><Camera size={15} /> תמונה</a>)}</div> : null}
                  <small>עודכן על ידי הצוות: {journal.staff_signature ?? "לא צוינה"}</small>
                </article>
              ))}
            </div>
          )}
