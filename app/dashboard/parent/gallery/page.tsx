...
export default async function Page() {
  ...
  const { data } = await supabase.from("gallery_items" as any).select("id, title, media_type, file_url, watermark_applied, created_at").eq("garden_id", profile.garden_id ?? "").eq("visible_to_parents", true).order("created_at", { ascending: false }).limit(60);
