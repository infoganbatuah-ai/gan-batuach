...
export default async function Page() {
  ...
  const permissionsRes = parentId ? await supabase.from("parent_camera_permissions" as any).select("id, allowed, valid_from, valid_until, reason, camera_streams(id, garden_id, kindergarten_id, name, area, camera_type, source_type, protocol, status, parent_view_allowed, parent_viewing_allowed, hls_playback_url, sample_hls_url, webrtc_playback_url, last_health_check_at, gardens(name))").eq("parent_id", parentId).eq("allowed", true).limit(30) : { data: [] };
