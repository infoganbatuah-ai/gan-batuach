...
export default async function Page() {
  ...
  const childGardenIds = scope.childGardenIds;
  ...
  const camerasByGardenId = childGardenIds.length ? await supabase.from("camera_streams" as any).select(selectColumns).in("garden_id", childGardenIds).limit(120) : { data: [] };
  const camerasByKindergartenId = childGardenIds.length ? await supabase.from("camera_streams" as any).select(selectColumns).in("kindergarten_id", childGardenIds).limit(120) : { data: [] };
  ...
  console.info("Parent camera query result", {
    ...
    kindergartenIdsFound: childGardenIds,
  ...
  const groups = childGardenIds.map((gardenId) => {
  ...
  const empty = !childGardenIds.length ? emptyState("no_relation") : allCameras.length === 0 ? emptyState("no_cameras") : emptyState("not_allowed");
