...
export default async function StaffMessagesPage({ searchParams }: { searchParams?: Promise<{ childId?: string }> }) {
  ...
  const [gardenRes, messagesRes, childrenRes] = await Promise.all([
    supabase.from("gardens" as any).select("manager:manager_id(id, full_name, email, role, profile_image_url), owner:owner_profile_id(id, full_name, email, role, profile_image_url)").eq("id", gardenId).maybeSingle(),
  ...
  const recipients = [garden?.manager, garden?.owner].filter(Boolean);
  return (
