...
export default async function StaffMessagesPage() {
  ...
  const [gardenRes, messagesRes] = await Promise.all([
  ...
  return <DashboardShell role="staff" title="הודעות צוות"><div className="dashboard-hero-card staff-hero-card"><div><p className="eyebrow">Staff Messaging</p><h1>תקשורת פנימית מתועדת.</h1><p>צוות יכול לפנות למנהלת, בעלים או פקח לפי הרשאה.</p></div><span className="pill good">מתועד</span></div><InternalMessagingCenter gardenId={gardenId} recipients={recipients} messages={(messagesRes.data ?? []) as any[]} /></DashboardShell>;
}
