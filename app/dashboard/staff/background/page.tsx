...
export default async function Page() {
  ...
  if (!employment || authority?.data !== true) return (
    <StaffAppFrame mode="candidate" active="profile">
      <StaffPageHero eyebrow="אישור עבודה" title="תעודות ובדיקות רקע" text="מסמכי מועמדות נבדקים בתהליך הקליטה. מסמכי עבודה לפי גן זמינים לאחר הפעלת העסקה." icon={ShieldCheck} />
      <StaffSection title="תהליך הקליטה"><a className="button secondary" href="/onboarding/staff">המשך קליטה</a></StaffSection>
    </StaffAppFrame>
  );
  const staffRes = await supabase.from("staff" as any).select("id, full_name, approved_to_work, police_clearance_status, background_check_status, role, created_at").eq("id", employment.staff_id).eq("garden_id", employment.garden_id).maybeSingle();
  const staff = staffRes.data as any;
  const docsRes = staff?.id ? await supabase.from("documents" as any).select("id, name, document_type, status, expires_at, file_url").eq("staff_id", staff.id).eq("garden_id", employment.garden_id).in("document_type", ["sexual_offense_clearance", "criminal_clearance", "police_clearance", "background_check"]).limit(20) : { data: [] };
