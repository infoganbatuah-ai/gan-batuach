...
export default async function InspectorInspectionHistoryPage() {
  ...
  const avg = rows.length ? Math.round(rows.reduce((sum, row) => sum + Number(row.weighted_score ?? 0), 0) / rows.length) : 91;
  ...
  return (
    <InspectorAppFrame profile={profileForUi} activeHref="/dashboard/inspector/reports" title="היסטוריית ביקורות" subtitle="עמידה בתקן, ציונים ודוחות עבר" badge="דוחות" backHref="/dashboard/inspector">
      ...
      <InspectorMetricGrid columns={4}>
        ...
        <InspectorMetricCard label="ציון ממוצע" value={avg} hint="מתוך 100" icon={BarChart3} />
        ...
        <InspectorList>
          {rows.map((row) => (
            <InspectorRow
              ...
              status={<><InspectorScoreRing value={row.weighted_score ?? "-"} label="ציון" /><InspectorStatus tone={Number(row.weighted_score ?? 0) >= 80 ? "success" : "danger"}>{row.completed_at ? new Date(row.completed_at).toLocaleDateString("he-IL") : row.status}</InspectorStatus></>}
