...
export default async function InspectorCamerasPage() {
  ...
  const camerasRes = gardenIds.length ? await supabase.from("camera_streams" as any).select("id, garden_id, kindergarten_id, name, area, camera_type, source_type, protocol, status, active, parent_view_allowed, parent_viewing_allowed, hls_playback_url, sample_hls_url, webrtc_playback_url, last_health_check_at, gardens(name, city)").in("garden_id", gardenIds) : { data: [] };
