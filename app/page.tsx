    ...
    <>
      ...
      <main>
        ...
        <section className="section compact-section">
          ...
          {homeGardens.length === 0 ? <div className="empty-state"><strong>אין עדיין גנים ציבוריים להצגה</strong><span>כאשר אדמין יפעיל פרופיל ציבורי לגן, הוא יוצג כאן.</span></div> : <div className="garden-card-grid">{homeGardens.map((garden: any) => <article className="public-garden-card" key={garden.id}>{garden.image_url ? <img className="garden-card-image" src={garden.image_url} alt={garden.name} /> : <div className="garden-image-placeholder">{garden.name}</div>}<div className="garden-card-top"><span className={garden.safe_status === "safe" ? "pill good" : "pill warn"}>{garden.safe_status ?? "pending_review"}</span><span><MapPin size={16} /> {garden.city}</span></div><h3>{garden.name}</h3><p>{garden.address ?? "כתובת תוצג לפי הרשאת הגן"} · מנהלת: {garden.manager?.full_name ?? garden.owner_name ?? "לא צוין"}</p><div className="garden-facts"><span>דירוג: {garden.rating ?? "חדש"}</span><span>מקבל: {formatAgeGroups(garden.supported_age_groups ?? [])}</span><span>{formatPublicPriceRange(garden.supported_age_groups ?? [])}</span><span>ילדים: {garden.current_children_count ?? 0}/{garden.children_capacity ?? 0}</span><span>ציון ביקורת: {garden.last_inspection_score ?? "טרם"}</span><span>ביקורת הבאה: {garden.next_inspection_at ? new Date(garden.next_inspection_at).toLocaleDateString("he-IL") : "טרם נקבע"}</span></div><div className="actions"><Link className="button primary" href={`/gardens/${garden.id}`}>צפייה בגן</Link><Link className="button secondary" href={`/gardens/${garden.id}#registration`}>בקשת רישום ילד</Link><Link className="button secondary" href="/login">כניסת הורים</Link><Link className="button" href="/login">כניסת צוות</Link></div></article>)}</div>}
        </section>
