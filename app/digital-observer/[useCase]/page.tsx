...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return item ? digitalObserverMetadata(item.path, `${item.title} | תצפיתן דיגיטלי`, `${item.problem} ${item.solution}`) : {};
}
...
export default async function DigitalObserverUseCasePage({ params }: PageProps) {
  ...
  return <main className="do-public" dir="rtl">
    ...
    <section className="do-public-trust"><ShieldCheck /><div><h2>מתחילים במצב בדיקה</h2><p>אין הפעלה של מצלמה, AI, הודעה או חיוב אמיתי לפני חיבור ספק מאושר ובדיקת הרשאות. אם נדרש בירור של אירוע מצולם, קראו גם על <Link href="/digital-observer/video-investigation">ניתוח וחקירת וידאו</Link>.</p></div><Link className="do-button secondary light" href="/digital-observer/trust">פרטיות ואמון</Link></section>
  </main>;
}
