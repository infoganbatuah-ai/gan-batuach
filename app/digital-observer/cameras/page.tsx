...
import { ObserverCameraNameForm, ObserverQuickAction } from "@/components/digital-observer/observer-action-forms";
...
function liveGatewayStreamId(camera: Record<string, any>) {
...
export default async function DigitalObserverCamerasPage({ searchParams }: PageProps) {
  ...
  const selectedHasLiveGateway = Boolean(site && selected && liveGatewayStreamId(selected));
    ...
    <div className={`do-page-stack do-camera-workspace do-camera-workspace-${mode}`}>
      ...
      {!selected ? <section className="do-camera-browser">
        ...
        {filteredCameras.length ? <div className={mode === "business" ? "do-business-camera-layout" : undefined}>
          ...
          const hasLiveGateway = Boolean(site && liveGatewayStreamId(camera));
          return <Link className="do-camera-live-tile" href={`/digital-observer/cameras?site=${site?.id}&camera=${camera.id}`} key={camera.id}>{hasLiveGateway ? <ObserverLivePlayer compact observerSiteId={site!.id} cameraSourceId={camera.id} name={camera.display_name || "מצלמה"} /> : <ObserverCameraMedia name={camera.display_name || "מצלמה"} mode={mode} scene={camera.preview_scene || sceneFor(originalIndex, mode)} status={camera.status || camera.health_status} sourceMode={camera.source_mode} />}<ObserverCameraPresence active={hasLiveGateway} /><span className="do-camera-tile-meta"><strong>{camera.display_name || "מצלמה"}</strong><small>{hasLiveGateway ? "שידור חי דרך Gateway" : camera.location_label || observerStatusLabel(camera.connector_type)}</small></span></Link>;
        })}</div>
