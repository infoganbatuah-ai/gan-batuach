    ...
    <div className={`do-page-stack do-camera-workspace do-camera-workspace-${mode}`}>
      ...
      {selected ? <section className="do-camera-detail-view">
        ...
        <article className="do-panel do-camera-player-panel">
          ...
          <ObserverCameraMedia large name={selected.display_name} mode={mode} scene={selected.preview_scene || sceneFor(cameras.indexOf(selected), mode)} status={selected.status || selected.health_status} sourceMode={selected.source_mode} />
          ...
          <div className="do-camera-readiness-strip"><CircleDot /><span><strong>צפייה מאובטחת במצב מוכנות</strong><small>וידאו חי, שמע והקלטה ייפתחו רק לאחר Gateway וטוקן קצר.</small></span><Wifi /></div>
        </article>
