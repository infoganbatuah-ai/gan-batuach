...
export default async function DigitalObserverCamerasPage({ searchParams }: PageProps) {
  ...
  const filteredCameras = cameraPool.filter((camera) => {
    ...
    const matchesStatus = !params?.status
      || params.status === "all"
      || (params.status === "active" ? digitalObserverCameraHasLiveStream(camera) : params.status === "offline" ? !digitalObserverCameraHasLiveStream(camera) : status === params.status);
  ...
  const selectedHasLiveGateway = Boolean(site && selected && digitalObserverCameraHasLiveStream(selected));
  ...
  return <ObserverAppShell profile={profile} mode={mode} activeHref="/digital-observer/cameras" title={mode === "home" ? "צפייה חיה" : "מצלמות"} statusLabel={`${connectedCount} מקורות משדרים`} mobileBackHref={selected ? camerasHref : undefined} actions={<Link className="do-button primary" href={site ? `/digital-observer/cameras/add?site=${site.id}` : "/digital-observer/onboarding"}><Plus /> הוספת מצלמות</Link>}>
    <div className={`do-page-stack do-camera-workspace do-camera-workspace-${mode}`}>
      ...
      {selected ? <section className="do-camera-detail-view">
        ...
        <article className="do-panel do-camera-player-panel">
          ...
          <div className="do-camera-readiness-strip"><CircleDot /><span><strong>{selectedHasLiveGateway ? "צפייה חיה מאובטחת" : "צפייה מאובטחת במצב מוכנות"}</strong><small>{selectedHasLiveGateway ? "הווידאו מתקבל דרך מתאם מאומת והרשאת צפייה קצרת־חיים." : "וידאו חי, שמע והקלטה ייפתחו רק לאחר חיבור מקור מאומת."}</small></span><Wifi /></div>
        </article>
        <aside className="do-camera-detail-side"><article className="do-panel do-form-section"><div className="do-section-head"><div><h2>פרטי המצלמה</h2><p>השם והחלל משמשים כהקשר לתצפיתן.</p></div><Settings2 /></div><ObserverCameraNameForm camera={selected} /><div className="do-summary-list"><div><span>סוג מערכת</span><strong>{observerStatusLabel(selected.connector_type)}</strong></div><div><span>דרך חיבור</span><strong>{observerStatusLabel(selected.connection?.connection_method || selected.connector_type)}</strong></div><div><span>מתאם</span><strong>{selected.connection?.adapter_type || "טרם נקבע"}</strong></div><div><span>מצב</span><strong>{observerStatusLabel(selected.status)}</strong></div><div><span>בריאות</span><strong>{observerStatusLabel(selected.connection?.health?.state || selected.health_status)}</strong></div><div><span>בדיקה אחרונה</span><strong>{formatObserverDate(selected.last_health_check_at)}</strong></div><div><span>מיקום</span><strong>{selected.location_label || "טרם הוגדר"}</strong></div><div><span>וידאו חי</span><strong>{selectedHasLiveGateway ? "מחובר דרך מתאם מאומת" : "לא פעיל"}</strong></div></div><div className="do-notice info"><LockKeyhole /><span>כתובת המקור, שם המשתמש, הסיסמה ו-secret reference אינם נשלחים לדפדפן.</span></div><div className="do-button-row"><ObserverQuickAction endpoint="/api/digital-observer/cameras" body={{ action: "test_readiness", id: selected.id }}><ShieldCheck /> בדיקת מוכנות</ObserverQuickAction><ObserverQuickAction endpoint="/api/digital-observer/cameras" body={{ action: "disable", id: selected.id }} confirmText="להשבית את מקור המצלמה?"><CameraOff /> השבתה</ObserverQuickAction></div></article>
      ...
      {!selected ? <section className="do-camera-connection-note"><LockKeyhole /><div><strong>Digital‑First: משתמשים קודם בציוד שכבר מותקן</strong><span>האשף בודק ספק ענן, חיבור ישיר, ONVIF/RTSP ומחבר תוכנה לפני המלצה על Gateway פיזי. פרטי גישה אינם נשמרים בדפדפן.</span></div><Link className="do-link" href={site ? `/digital-observer/cameras/add?site=${site.id}` : "/digital-observer/onboarding"}>פתיחת אשף החיבור</Link></section> : null}
    </div>
  </ObserverAppShell>;
}
