...
const matchesSeverity = (value: string, filter: string) => ["info", "low"].includes(filter) ? ["info", "low"].includes(value) : filter === "critical" ? ["critical", "urgent"].includes(value) : value === filter;
...
export default async function DigitalObserverAlertsPage({ searchParams }: PageProps) {
  ...
  const spatialMismatches = site ? runtime.spatiallyRejectedSignals.filter(item => item.observer_site_id === site.id) : [];
  ...
  const signals = displayableSignals.filter((item) => {
    if (params?.severity && !matchesSeverity(item.severity, params.severity)) return false;
  ...
  const selected = params?.event ? [...allSignals, ...spatialMismatches].find((item) => item.id === params.event) ?? null : null;
  ...
  const selectedMediaFault = selected?.metadata?.media_fault && typeof selected.metadata.media_fault === "object" ? selected.metadata.media_fault : null;
  ...
  return <ObserverAppShell profile={profile} mode={mode} activeHref="/digital-observer/alerts" title={selected ? "פרטי אירוע" : timeline ? "אירועים" : "מרכז ההתראות"} statusLabel="ביקורת אנושית חובה" desktopSearch={{ action: "/digital-observer/alerts", placeholder: "חיפוש אירועים, מצלמות ומיקומים..." }}>
    <div className="do-page-stack do-alert-center">
      <ObserverJournalRefresh latestId={allSignals[0]?.id} severity={allSignals[0]?.severity} />
      {spatialMismatches.length ? <section className="do-panel" aria-label="אירועים שאינם תואמים לחלל"><h2>אירועים שאינם תואמים לחלל</h2><p>{spatialMismatches.length} רשומות אינן נספרות ביומן הפעיל. ההיסטוריה והחלטות הבדיקה המקוריות נשמרו; הסימון הוא בדיקת התאמה למיפוי החלל, לא החלטה אנושית על האירוע.</p><ul>{spatialMismatches.map(item => <li key={item.id}><Link href={`/digital-observer/alerts?${new URLSearchParams({ site: site!.id, event: item.id })}`}>{item.metadata?.camera_name} · {observerEventLabel(item.metadata?.event_type ?? item.signal_type)} · {formatObserverDate(item.created_at)}</Link></li>)}</ul></section> : null}
      {selected?.metadata?.journal_spatial_mismatch ? <p className="do-notice warn" role="status">סוג האירוע אינו תואם לחלל המצלמה. זוהי רשומה היסטורית שאינה נכללת ביומן הפעיל; אין להסיק ממנה שהזיהוי אומת.</p> : null}
      {!selected ? <>
        ...
        })}</div> : <div className="do-empty"><CheckCircle2 /><strong>אין אירועים להצגה</strong><span>אירועים מאומתים יופיעו עם מקור מצלמה; הקלטה נשמרת רק לפי הצורך.</span></div>}
          </article>
        ...
        <section className="do-event-detail-screen">
          ...
          <div className="do-event-detail-grid" id="event-evidence">
            <article className="do-panel do-event-evidence">
              ...
              </div> : <div className="do-event-placeholder"><ShieldAlert /><strong>{selected.metadata?.recording_required === false ? "לא נדרשה הקלטה לאירוע זה" : selectedMediaFault?.status === "waived" ? "תקלה במדיה נסגרה בוויתור מתועד" : "מדיית האירוע טרם זמינה"}</strong><span>{selected.metadata?.recording_required === false ? "האירוע נרשם ללא שמירת וידאו בהתאם למדיניות." : selected.metadata?.media_missing_reason || selectedClip?.media_missing_reason || selectedClipMetadata.media_missing_reason || "האירוע נשאר זמין לבדיקה גם כאשר לכידת המדיה נכשלה."}</span></div>}
              <div className="do-event-evidence-track" aria-label="ציר זמן הראיה">
                <div><strong>ציר זמן הראיה</strong><span>{selectedHasMedia ? `${selectedClip?.window_seconds_before ?? selectedClipMetadata.window_seconds_before ?? 0} שניות לפני · ${selectedClip?.window_seconds_after ?? selectedClipMetadata.window_seconds_after ?? 0} שניות אחרי` : "האירוע זמין לבדיקה גם ללא הקלטה"}</span></div>
            ...
            <div className="do-event-detail-actions do-button-row"><Link className="do-button secondary full" href={`/digital-observer/alerts?event=${selected.id}&preview=notification`}><Bell /> תצוגת התראה לנייד</Link>{selectedHasMedia ? <a className="do-button secondary full" href={`/api/digital-observer/event-clips/${selectedClip?.id}/media?kind=clip&download=1`}><ArrowDownToLine /> הורדת וידאו</a> : null}{(selectedHasMedia || selected.metadata?.validated_event) ? <ObserverQuickAction endpoint="/api/digital-observer/events/review" body={{ signal_id: selected.id, review_status: "confirmed", note: "אושר בביקורת אנושית" }}><CheckCircle2 /> אישור אירוע</ObserverQuickAction> : null}{(selectedHasMedia || selected.metadata?.validated_event) ? <ObserverQuickAction endpoint="/api/digital-observer/events/review" body={{ signal_id: selected.id, review_status: "dismissed", note: "הוסר מתור הבדיקה; לא נרשם Ground Truth" }}><ShieldCheck /> הסרה מתור הבדיקה</ObserverQuickAction> : null}{selectedMediaFault?.status === "open" ? <ObserverQuickAction endpoint="/api/digital-observer/events/review" confirmText="האירוע יישאר ביומן והוויתור יתועד בהיסטוריה הבלתי־נמחקת. להמשיך?" body={{ signal_id: selected.id, review_status: "resolved", media_fault_action: "waive", note: "התקלה נסגרה ללא מדיה לאחר בדיקה אנושית" }}><ShieldCheck /> סגירת תקלה ללא מדיה</ObserverQuickAction> : null}<ObserverQuickAction endpoint="/api/digital-observer/events/review" body={{ signal_id: selected.id, review_status: "escalated", note: selectedHasMedia ? "הועבר להמשך בדיקה" : "מדיית האירוע חסרה - תקלה טכנית" }}><AlertTriangle /> {selectedHasMedia ? "העברה לבדיקה אנושית" : "סימון תקלה טכנית"}</ObserverQuickAction></div>
          </div>
        </section>
      ) : null}
    </div>
