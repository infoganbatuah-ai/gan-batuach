...
export default async function DigitalObserverAdminOperationsPage() {
  const { profile } = await requireDigitalObserverAdmin("/digital-observer/admin/operations");
  ...
  return <ObserverAppShell profile={profile} mode="admin" activeHref="/digital-observer/admin/operations" title="תפעול ובריאות מערכת" statusLabel="אין סודות בדפדפן">
    <div className="do-page-stack">
      ...
      <section className="do-grid cols-2"><article className="do-panel" id="policies"><h2>מדיניות וכללי ניטור</h2><div className="do-notice info"><ShieldCheck /><span>שינויים במדיניות נשארים בביקורת אנושית. אין הסלמה או חיוג אוטומטי.</span></div></article><article className="do-panel" id="learning"><h2>למידה ומדדי איכות</h2><div className="do-notice info"><BrainCircuit /><span>זמן לבדו אינו מוכיח אמינות. הרחבת אוטומציה מותנית בתוצאות מאומתות, התרעות שווא וניטור ירידת איכות.</span></div></article></section>
    </div>
  </ObserverAppShell>;
}
