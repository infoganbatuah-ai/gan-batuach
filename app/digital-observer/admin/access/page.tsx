...
export default async function DigitalObserverAdminAccessPage() {
  const { profile } = await requireDigitalObserverAdmin("/digital-observer/admin/access");
  const supabase = createDigitalObserverAdminDataClient();
  const [sites, memberships, organizations] = await Promise.all([
    supabase.from("observer_sites" as any).select("id,name,site_type,city,formatted_address,active,owner_profile_id,created_at").is("garden_id", null).neq("site_type", "kindergarten").order("created_at", { ascending: false }).limit(300),
  ...
  return <ObserverAppShell profile={profile} mode="admin" activeHref="/digital-observer/admin/access" title="לקוחות והרשאות" statusLabel="Multi-tenant">
    <div className="do-page-stack">
      ...
      <div className="do-notice info"><ShieldCheck /><span>האדמין רואה מטא-דאטה של שיוך בלבד. מסך זה אינו מבצע התחזות ואינו מציג סיסמאות, וידאו או פרטי חיבור.</span></div>
