...
export async function POST(request: Request) {
  try {
    ...
    const [{ data: existing, error: existingError }, { data: garden, error: gardenError }] = await Promise.all([
      ...
      admin.from("gardens" as any).select("id,name").eq("id", access.gardenId).single()
    ]);
    ...
    if (existing) {
      const activeLink = await admin.from("parent_kindergarten_links" as any).select("id").eq("parent_profile_id", existing.id).eq("garden_id", access.gardenId).eq("status", "active").maybeSingle();
      if (activeLink.data) return fail("ההורה כבר משויך לגן.", 409);
    }

    let priceSnapshot: number | null = null;
    if (payload.fee_group_id) {
      const fee = await admin.from("kindergarten_fee_groups" as any).select("id,monthly_fee").eq("id", payload.fee_group_id).eq("garden_id", access.gardenId).maybeSingle();
    ...
    const legacy = await admin.from("user_affiliation_requests" as any).insert({
      requester_id: profile.id, target_type: "kindergarten", target_id: access.gardenId,
    ...
    try {
      signed = await createSignedInvitation(admin, {
        invitationType: "parent_guardian", intendedRole: "parent", gardenId: access.gardenId,
    ...
    const delivery = await deliverSignedInvitation(admin, { invitationId: signed.invitation.id, email, recipientName: payload.full_name, gardenId: access.gardenId, gardenName: garden.name, targetProfileId: existing?.id ?? null, url: signed.url });
    await admin.from("audit_logs" as any).insert({
      actor_id: profile.id, actor_role: profile.role, garden_id: access.gardenId,
