...
const onboardingSchema = z.object({
  ...
  activation_key: z.string().uuid().optional(),
  consents_accepted: z.boolean().optional(),
  garden: z.object({
...
export async function GET(request: Request) {
  try {
    ...
    const { data, error } = await supabase.rpc("garden_onboarding_status" as never, { target_garden_id: gardenId } as never);
    if (error) return fail(error.code === "42501" ? "אין הרשאה לתהליך הקליטה הזה" : "לא ניתן לטעון את מצב הקליטה", error.code === "42501" ? 403 : 400);
    return ok(data);
...
export async function PATCH(request: Request) {
  try {
    ...
    const [{ data: existingGarden }, { data: existingOnboarding }] = await Promise.all([supabase
      .from("gardens" as any)
      ...
      .eq("id", gardenId)
      .maybeSingle(), supabase.from("kindergarten_onboarding_records" as any).select("profile_data,activation_key,registrant_type").eq("garden_id", gardenId).maybeSingle()]);
    ...
    const merged = { ...(existingGarden ?? {}), ...((existingOnboarding as any)?.profile_data ?? {}), ...profileData };
    ...
    const gardenPatch = Object.fromEntries(Object.entries({
      ...
      admin_correction_note: existingGarden?.admin_correction_note ?? null,
      updated_at: now
    ...
    const { data: onboarding, error: onboardingError } = await supabase
      ...
      .upsert({
        ...
        subscription_monthly_amount: subscriptionAmount,
        correction_note: existingGarden?.admin_correction_note ?? null,
        current_step: activationWizardSteps[Math.min(activationWizardSteps.length - 1, Math.max(0, Number(payload.garden.registration_step ?? 1) - 1))],
        registrant_type: (existingOnboarding as any)?.registrant_type ?? "teacher_operator",
        started_at: now,
        updated_at: now
    ...
    if (payload.submit) {
      if (!payload.consents_accepted) return fail("נדרש לאשר את התנאים ומדיניות הפרטיות לפני ההפעלה", 422);
      const activationKey = payload.activation_key ?? (existingOnboarding as any)?.activation_key;
      if (!activationKey) return fail("מפתח ההפעלה חסר. יש לרענן את התהליך.", 409);
      const activation = await sessionClient.rpc("activate_garden_onboarding" as never, { target_garden_id: gardenId, requested_activation_key: activationKey, accept_required_consents: true } as never);
      if (activation.error) return fail(activation.error.message.includes("requirements_not_met") ? "לא ניתן להפעיל: נותרו דרישות חובה" : "הפעלת הגן נכשלה בבטחה", activation.error.code === "42501" ? 403 : 422);
      revalidatePath("/dashboard/garden");
      return ok({ garden: { ...garden, approval_flow_status: "active" }, onboarding: activation.data, missing: [], next_path: `/api/management/gardens` });
    }

    revalidatePath("/onboarding/kindergarten");
    revalidatePath("/dashboard/admin/leads");
    revalidatePath("/dashboard/garden");
    return ok({ garden, onboarding, missing: progress.missingLabels, next_path: `/onboarding/kindergarten?gardenId=${gardenId}` });
  } catch (error) {
