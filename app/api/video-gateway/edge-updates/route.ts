...
const statusSchema = z.object({ release_id: z.string().regex(/^[A-Za-z0-9._:-]{3,160}$/),
  state: z.enum(["UPDATE_AVAILABLE","DOWNLOADING","VERIFYING","STAGED","INSTALLING","RESTARTING","VERIFYING_HEALTH","HEALTHY","ROLLBACK_REQUIRED","ROLLING_BACK","ROLLED_BACK","UPDATE_FAILED","ACTION_REQUIRED"]),
...
export async function POST(request: Request) {
  try {
    ...
    const firstTerminalReport = !["HEALTHY","ROLLED_BACK","UPDATE_FAILED","ACTION_REQUIRED"].includes(previous.data?.state || "")
      && ["HEALTHY","ROLLED_BACK","UPDATE_FAILED","ACTION_REQUIRED"].includes(payload.state);
    ...
    if (rollout.data && firstTerminalReport && rollout.data.stage === "CANARY" && ["ROLLED_BACK","UPDATE_FAILED","ACTION_REQUIRED"].includes(payload.state)) {
