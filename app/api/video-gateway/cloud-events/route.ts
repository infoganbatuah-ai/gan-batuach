...
export async function POST(request: Request) {
  try {
    ...
    const [site, source] = await Promise.all([
      ...
      db.from("digital_observer_camera_sources").select("id,display_name,location_label,metadata,status,source_mode").eq("id", event.camera_source_id).eq("observer_site_id", device.observer_site_id).maybeSingle()
    ]);
    ...
    const severity = { INFO: "info", WARNING: "medium", CRITICAL: "critical" }[result.event.severity];
    ...
    if (signal.data.metadata.camera_source_id !== camera.id || signal.data.metadata.event_type !== result.event.event_type || signal.data.metadata.first_seen !== event.timestamp) return fail("Event id was already used for a different observation.", 409);
    if (event.media_failure_reason && signal.data.metadata.recording_required && signal.data.metadata.media_status !== "available") {
      const observedAt = new Date().toISOString();
      const lifecycle = openMediaFault(signal.data.metadata, event.media_failure_reason, observedAt);
      ...
      const write = await db.from("observer_intelligence_signals").update({ metadata: updated }).eq("id", signal.data.id).eq("observer_site_id", device.observer_site_id);
      if (write.error) throw new Error("EVENT_MEDIA_STATUS_FAILED");
      signal.data.metadata = updated;
      if (lifecycle.transition === "opened") await writeAuditEvent({
        ...
        targetId: signal.data.id, cameraId: camera.id, riskLevel: "medium",
        metadata: { observer_site_id: device.observer_site_id, status: "open", reason: lifecycle.fault.reason, occurred_at: observedAt }
      });
    }
    const notifications = await recordEventNotifications(db, device.observer_site_id, signal.data.id, signal.data.severity);
    // An in-app alert exists as soon as the signal is persisted. External delivery is a separate provider workflow.
    return ok({ status: "stored", signal_id: signal.data.id, recording_required: signal.data.metadata.recording_required,
      media_event_id: sourceId, media_status: signal.data.metadata.media_status, in_app_available: true, notifications_pending: notifications.push_pending }, 201);
