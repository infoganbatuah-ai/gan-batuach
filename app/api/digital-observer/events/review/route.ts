...
export async function POST(request: Request) {
  try {
    ...
    const signalResult = await supabase.from("observer_intelligence_signals" as never)
      ...
      .maybeSingle();
    const signal = signalResult.data as unknown as SignalRow | null;
    ...
    const { data: review, error: reviewError } = await supabase.from("observer_signal_reviews" as never).insert({
      signal_id: signal.id,
    ...
    } as never).select("id,review_status,created_at").single();
    if (reviewError) return fail("לא ניתן לשמור את ביקורת האירוע.", 400);

    const { data: updated, error } = await supabase.from("observer_intelligence_signals" as never).update({
      review_status: payload.review_status,
      reviewed_at: now,
    ...
    } as never).eq("id", signal.id).select("id,review_status,reviewed_at,resolved_at").single();
    ...
    if (mediaFault?.transition === "waived") await writeAuditEvent({
      ...
      cameraId: typeof signal.metadata?.camera_source_id === "string" ? signal.metadata.camera_source_id : null, riskLevel: "medium",
