...
## Tests
Pre-merge checks: all 199 Management regression/focused tests pass, including six GB-M24 contract/security tests; typecheck, production build, domain QA (29), security QA (7), migration audit and lint-baseline with zero new regressions pass. A rollback-only migration execution against the linked database passes. A rollback-only behavioral probe cannot execute newly added enum values inside the same transaction because PostgreSQL requires the enum addition to commit first; it must run immediately after the production migration. That probe covers eligible/ineligible assignees, Garden isolation, candidate denial, corrective-source preservation and idempotent transitions. A true independent-connection completion race requires controlled live identities; row-level `FOR UPDATE` is the database serialization boundary.
