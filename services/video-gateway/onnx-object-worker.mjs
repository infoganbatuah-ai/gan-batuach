...
function startupProgress(phase) {
  if (command === "--serve-rgb" && process.connected) {
    try { process.send?.({ type: "startup_progress", phase }, () => {}); } catch { /* Diagnostics never abort model startup. */ }
  }
}
if (!["--self-test", "--infer-rgb", "--serve-rgb"].includes(command)) {
  process.stderr.write("usage: onnx-object-worker --self-test|--infer-rgb|--serve-rgb\n");
  process.exit(64);
}

startupProgress("model_validation");
...
try {
  startupProgress("runtime_loading");
  ...
  // Keep CPU pools bounded for live relays. The sequential session was observed
  // stuck in FindReusableTensor before self-test; ORT_PARALLEL skips that reuse
  // planner path. Two inter-op threads are required: one can fall back to the
  // sequential executor. Intra-op stays single-threaded to bound total work.
  startupProgress("session_loading");
  ...
  if (valueCount !== 300 * 300 * 3) throw new Error("unsupported_object_model_dimensions");
  async function infer(inputBytes) {
    ...
    const input = new ort.Tensor(metadata.type, tensorData, dimensions);
  ...
  if (command === "--serve-rgb") {
    if (!process.send) throw new Error("object_worker_ipc_required");
    startupProgress("self_test");
    await infer(null);
    let busy = false;
    process.on("disconnect", () => process.exit(0));
    process.on("message", async (message) => {
      if (!message || message.type !== "infer" || typeof message.id !== "string") return;
      if (busy || typeof message.rgb !== "string" || message.rgb.length !== valueCount * 4 / 3) {
        process.send?.({ type: "result", id: message.id, ok: false });
        return;
      }
      busy = true;
      try {
        const detections = await infer(Buffer.from(message.rgb, "base64"));
        if (process.connected) process.send({ type: "result", id: message.id, ok: true, detections, no_raw_frame_returned: true });
      } catch {
        if (process.connected) process.send({ type: "result", id: message.id, ok: false });
      } finally {
        busy = false;
      }
    });
    process.send({ type: "ready", ok: true, provenance, inference_self_test: true });
  } else {
  ...
  const detections = await infer(inputBytes);
