...
const PAYLOAD_FIELDS = Object.freeze({
  lighting: new Set(["enabled", "level", "duration_ms"]),
...
function payloadFor(action, payload) {
  ...
  if (action === "lighting") {
    ...
    if (payload.duration_ms !== undefined && (!payload.enabled || !Number.isInteger(payload.duration_ms)
      || payload.duration_ms < 1_000 || payload.duration_ms > MAX_LIGHTING_PULSE_MS)) {
      throw failure("invalid_lighting_duration");
    }
...
export function createPrivateNvrCommandExecutor({
  ...
  async function execute(request, identity) {
    ...
    try {
      ...
      let result;
      if (request.action === "lighting") {
        if (request.payload.enabled && request.payload.duration_ms) {
          let startError = null;
          try {
          ...
          if (phase.writeAttempted) {
            result = await verifiedAlarmWrite(session, request, false, { continuationStop: true, phase, ensureDispatchAllowed }).catch((error) => {
              throw failure("lighting_stop_state_unknown", { cause: error?.code || "unknown" });
            });
          }
...
export const privateNvrCommandContract = Object.freeze({
  ...
  write_paths: Object.freeze([PATHS.alarmSet]),
  read_paths: Object.freeze([...new Set([PATHS.alarmGet, PATHS.ptzRange])]),
  supported_actions: Object.freeze(["lighting", "siren"]),
  blocked_actions: Object.freeze({
    ptz: "not_authorized_for_installed_executor",
    talkback: "vendor_media_transport_not_safely_derived"
  }),
