...
export function createPrivateNvrPreflightDriver({ resolveSource, probe, now = Date.now }) {
  ...
  return async function handle(request, identity) {
    ...
    const result = (async () => {
      ...
      const common = { camera_id: request.camera_id, site_id: request.site_id, stream_id: request.stream_id, channel: request.channel, executor_installed: false, executed: false, evidence_id: randomUUID(), verified_at: tested ? verifiedAt : null };
